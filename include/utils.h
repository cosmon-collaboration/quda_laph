...
void xmlsetQLInt(XMLHandler &xmlin, const std::string &tagname,
                 std::vector<int> &ivalues, int &index, bool optional = false,
                 int default_value = 0);
...
XMLHandler xmloutputQLReal(const std::string &tagname,
                           const std::vector<double> &rvalues, int &index);
...
int xmlputQLInt(const std::string &tagname, const std::vector<int> &ivalues,
                int &index);

bool xmlputQLBool(const std::string &tagname, const std::vector<int> &ivalues,
                  int &index);
...
//  and returns the XML content descending under it, returning an XMLHandler
//  with "tagname" as the root; if "tagname" is not found, then an XMLHandler is
//  returned with just the root tag of "tagname".

#ifndef IO_FM_MAP_LAPH_H
#define IO_FM_MAP_LAPH_H
...
// *    (2) a numbytes(ioh,K) function, where ioh is an IOFMHandler, must be *
...
// *  an object is created on every rank.  In global mode, lattice and other *
// *  distributed data can be read and written, and common data is written by
// the  *
// *  primary rank and read by the primary rank with subsequent broadcast to all
// *
// *  ranks.  In local mode, the object is assumed to exist only on the current
// *
// *  rank.  Only non-distributed data can be read and written.  Read/writes are
// *
// *  done through the current rank, and no data is broadcast to other ranks. *
// * *
...
// *
// *  the header string).  NOTE: in global mode, only the primary rank gets the
// *
// *  header string; all other ranks will only receive the empty string. *
// * *
...
class TwoSpin
{
  ...
  public:
    ...
    TwoSpin(int in1, int in2) {assign(in1,in2);} // no default constructor by
design !!
...
inline void TwoSpin::assign(int in1, int in2)
{
 if ((in1<0)||(in1>3)||(in2<0)||(in2>3)){
    errorLaph(make_str("Invalid TwoSpin initialization"));}
...
template <typename T>
void multi_write(IOFMHandler &ioh, const std::vector<T> &output) {
  int n = output.size();
  if (n < 1)
    return;
  int nint = T::numints();
...
template <typename T>
void multi_read(IOFMHandler &ioh, std::vector<T> &input, int n) {
  input.clear();
  if (n < 1)
    return;
  input.reserve(n);
...
inline size_t numbytes(IOFMHandler &ioh, const UIntKey &uikey) {
...
template <typename K, typename V> class IOMap {
  ...
  typedef IOFMHandler::pos_type pos_type;
  typedef IOFMHandler::off_type off_type;
  typedef unsigned long long w_pos;

  mutable IOFMHandler ioh;
...
public:
  typedef IOFMHandler::OpenMode OpenMode;
...
private:
  void initialize(const std::string &filename, OpenMode mode,
                  ...
                  int striping_factor, int striping_unit, bool turn_on_checksum,
                  std::string &header, bool read_header, bool overwrites);
...
IOMap<K, V>::IOMap(bool global_mode)
    : ioh(global_mode), checksums_in_file(false), use_checksums(false),
...
void IOMap<K, V>::openReadOnly(const std::string &filename,
  ...
  initialize(filename, mode, filetype_id, 'N', 1, 0, turn_on_checksum, header,
             true, false);
}
...
void IOMap<K, V>::openNew(const std::string &filename,
  ...
  initialize(filename, mode, filetype_id, endianness, striping_factor,
             striping_unit, turn_on_checksum, tmp_header, false,
             overwrites_allowed);
}
...
void IOMap<K, V>::openUpdate(const std::string &filename,
  ...
  initialize(filename, mode, filetype_id, endianness, striping_factor,
             striping_unit, turn_on_checksum, header, true, overwrites_allowed);
}
...
void IOMap<K, V>::initialize(const std::string &filename, OpenMode mode,
  ...
  if (verbose2) {
    ...
    if (ioh.isGlobal())
      std::cout << "   IOMap is global" << std::endl;
    else
      std::cout << "   IOMap is local on rank " << LayoutInfo::getMyRank()
                << std::endl;
...
template <typename K, typename V> void IOMap<K, V>::setHighVerbosity() {
  verbose1 = verbose2 = (ioh.isLocal()) || (isPrimaryRank());
}

template <typename K, typename V> void IOMap<K, V>::setMediumVerbosity() {
  verbose1 = (ioh.isLocal()) || (isPrimaryRank());
  verbose2 = false;
}

...
// ************************************************************
// *                                                          *
// *   The class "Array" is defined in this file.             *
// *                                                          *
// ************************************************************
...
public:
  ...
  uint rightmostSize() const { return m_sizes.back(); }
...
private:
  ...
  unsigned int serialindex(const std::vector<int> &ind, int istart) const;
  unsigned int serialindex(const std::vector<unsigned int> &ind,
                           unsigned int istart) const;
  const T &get(unsigned int serind) const;

  const T *dp(uint k) const { return &(m_store[k]); }
...
#ifdef SAFETY_FLAG
  // if (m_sizes.size()!=1) throw(std::invalid_argument("Array: improper rank of
  // array indices"));
...
template <typename T>
inline unsigned int Array<T>::serialindex(const std::vector<int> &ind,
                                          int istart) const {
  uint off = ind[istart + m_sizes.size() - 1];
  for (int i = int(m_sizes.size()) - 2; i >= 0; --i)
    off = off * m_sizes[i] + ind[istart + i];
  return off;
}

template <typename T>
inline unsigned int Array<T>::serialindex(const std::vector<uint> &ind,
                                          uint istart) const {
  uint off = ind[istart + m_sizes.size() - 1];
  for (int i = int(m_sizes.size()) - 2; i >= 0; --i)
    off = off * m_sizes[i] + ind[istart + i];
...
template <typename T> inline const T &Array<T>::get(unsigned int off) const {
  return m_store[off];
}

template <typename T> inline T &Array<T>::getFront() { return m_store[0]; }
...
template <typename T> inline Array<T> &Array<T>::conj() {
  for (uint i = 0; i < m_store.size(); i++)
    m_store[i] = std::conj(m_store[i]);
  return *this;
}

// *************************************************************

//  convenient routine for inserting into a map.  Throws exception
//  if key already in map.  Returns a pointer to the inserted value.

template <typename K, typename V>
V *map_insert_new(std::map<K, V> &the_map, const K &key, const V &value) {
  std::pair<typename std::map<K, V>::iterator, bool> rt =
      the_map.insert(std::make_pair(key, value));
  if (!(rt.second))
    throw(std::invalid_argument("Map insertions failed: key already in map"));

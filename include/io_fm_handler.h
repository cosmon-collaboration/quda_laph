...
public:
  ...
  DistArrayViewInfo(int time_slice,
                    IOH_int bytes_per_site); // for a lattice time slice
  ...
  DistArrayViewInfo(const std::vector<int> &gsizes,
                    const std::vector<int> &numranks,
                    IOH_int bytes_per_element);

  DistArrayViewInfo(const std::vector<int> &gsizes,
                    const std::vector<int> &numranks, int major_index,
                    IOH_int bytes_per_element);
  ...
  int getArrayDimensions() const { return ndim; }

  const std::vector<int> &getGlobalArraySizes() const { return global_sizes; }

  const std::vector<int> &getLocalArraySizes() const { return local_sizes; }

  const std::vector<int> &getViewLocalStarts() const { return start_indices; }

  IOH_int getBytesPerSite() const { return nbytes_per_element; }

  IOH_int getBytesPerElement() const { return nbytes_per_element; }

  IOH_int getViewElementsThisRank() const { return nelem_this_rank; }

  IOH_int getViewBytesThisRank() const {
    return nbytes_per_element * nelem_this_rank;
  }

  IOH_int getElementOffsetThisRank() const { return el_offset_this_rank; }

  IOH_int getViewTotalBytes() const { return gvvol * nbytes_per_element; }

  IOH_int getViewLexicoStart() const // in terms of elements
  {
    return lexico_start;
  } // helps with check sums
  ...
  const MPI_Datatype &getFileViewType() const { return ftype; }

  const MPI_Datatype &getFileElemType() const { return etype; }
...
public:
  ...
  const std::vector<int> &getGlobalArraySizes() const { return local_sizes; }

  const std::vector<int> &getLocalArraySizes() const { return local_sizes; }

  const std::vector<int> getViewLocalStarts() const {
    std::vector<int> tmp(ndim, 0);
    return tmp;
  }
  ...
  IOH_int getViewElementsThisRank() const { return gvvol; }

  IOH_int getViewBytesThisRank() const { return nbytes_per_element * gvvol; }

  IOH_int getElementOffsetThisRank() const { return lexico_start; }
...
public:
  ...
  template <typename T> void write(const T &output); // basic types

  template <typename T> void multi_write(const T *output, int n); // basic types

  template <typename T>
  void write(const std::vector<T> &output); // T is basic type

  template <typename T> void write(const Array<T> &output);
  ...
  template <typename T> void read(T &input); // basic types

  template <typename T> void multi_read(T *input, int n); // basic types

  template <typename T> void read(std::vector<T> &input); // basic types

  template <typename T> void read(Array<T> &input); // basic types
  ...
  template <typename T> size_t numbytes(const T &output); // basic types

  template <typename T>
  size_t numbytes(const std::vector<T> &output); // T is basic type

  template <typename T> size_t numbytes(const Array<T> &output); // basic types
...
template <typename T> void IOFMHandler::write(const T &output) {
  IOFMHandler::BasicType dummy(
      output); // cause compiler error if T is not a valid basic type
  write_common((const char *)&output, sizeof(T), 1);
}

template <typename T> void IOFMHandler::multi_write(const T *output, int n) {
  IOFMHandler::BasicType dummy(
      *output); // cause compiler error if T is not a valid basic type
  write_common((const char *)output, sizeof(T), n);
}

template <typename T> void IOFMHandler::write(const std::vector<T> &output) {
  IOFMHandler::BasicType dummy(
      output[0]); // cause compiler error if T is not a valid basic type
  int n = output.size();
  write(n);
  if (n == 0)
    return;
...
template <typename T> void IOFMHandler::write(const Array<T> &output) {
...
template <typename T> void IOFMHandler::read(T &input) {
  IOFMHandler::BasicType dummy(
      input); // cause compiler error if T is not a valid basic type
  read_common((char *)&input, sizeof(T), 1);
}

template <typename T> void IOFMHandler::multi_read(T *input, int n) {
  IOFMHandler::BasicType dummy(
      *input); // cause compiler error if T is not a valid basic type
  read_common((char *)input, sizeof(T), n);
}

template <typename T> void IOFMHandler::read(std::vector<T> &input) {
  IOFMHandler::BasicType dummy(
      input[0]); // cause compiler error if T is not a valid basic type
  int n;
  read(n);
  if (n == 0)
    return;
  ...
  check_for_failure((n < 0) || (n > 16777216),
                    "std::vector too large...bad file location?");
  input.resize(n);
  read_common((char *)&input[0], sizeof(T), size_t(n));
}

template <typename T> void IOFMHandler::read(Array<T> &input) {
  unsigned int n;
  read(n);
  if (n == 0)
    return;
...
template <typename T> void read(IOFMHandler &io, std::vector<T> &input) {
  io.read(input);
}
...
inline void read(IOFMHandler &io, LattField &input) { io.read(input); }
...
template <typename T> size_t IOFMHandler::numbytes(const T &data) {
  IOFMHandler::BasicType dummy(
      data); // cause compiler error if T is not a valid basic type
  return sizeof(T);
}

template <typename T> size_t IOFMHandler::numbytes(const std::vector<T> &data) {
  IOFMHandler::BasicType dummy(
      data[0]); // cause compiler error if T is not a valid basic type
  return sizeof(int) + data.size() * sizeof(T);
}

template <typename T> size_t IOFMHandler::numbytes(const Array<T> &data) {
...
template <>
inline size_t numbytes(IOFMHandler &io, const unsigned short int &data) {
  return io.numbytes(data);
}
...
template <>
inline size_t numbytes(IOFMHandler &io, const unsigned long int &data) {
  return io.numbytes(data);
}
...
template <>
inline size_t numbytes(IOFMHandler &io, const unsigned long long int &data) {
  return io.numbytes(data);
}
...
template <typename T> size_t numbytes(IOFMHandler &io, const Array<T> &data) {
  return io.numbytes(data);
}

...
// *
// *   inserting data into IOMap records in files specified in a FileListInfo
// and  *
...
// *
// *   the "put" and "get" classes simultaneously;  use "put" to build up the
// data *
// *   in the files, then destroy the insert object; after this, a "get" object
// *
// *   can be created to access the  data in the files.  Objects of these
// classes  *
...
// *          XMLHandler and an output(XMLHandler&) member; you need to specify
// the*
// *          header tag that the file key will be enclosed in; the file key
// must  *
...
// * *
// * *
// *   The "DataPutHandlerSF" and "DataGetHandlerSF" classes are similar to
// their  *
...
public:
  ...
  virtual void putData(const R &rkey,
                       const D &data) = 0; // insert into current file
...
template <typename H, typename R, typename D> class DataGetHandlerBaseSF {
public:
...
template <typename H, typename R, typename D> class DataPutHandlerBaseSF {
public:
...
class DataGetHandlerMF : public DataGetHandlerBaseMF<H, F, R, D> {
  ...
  struct StorageKey {
    F fkey;
    R rkey;
    StorageKey(const F &in_fkey, const R &in_rkey)
        : fkey(in_fkey), rkey(in_rkey) {}
...
public:
  DataGetHandlerMF(H &in_handler, const FileListInfo &in_filelist,
                   ...
                   const std::string &header_tag, bool global_mode = true,
    ...
    try {
      ...
      if (it != fileMap.end()) {
        fail(std::string("duplicate keys in fileMap in current Handler\n") +
             ...
             int_to_string(suffix) + " and suffix " +
             int_to_string((it->second).suffix) + " have same file key\n");
      }
...
void DataGetHandlerMF<H, F, R, D>::fail(const F &fkey, const R &rkey) {
  if ((isPrimaryRank()) || (!gmode)) {
    std::cout << "DataGetHandlerMF could not find requested record:"
              << std::endl;
...
void DataGetHandlerMF<H, F, R, D>::fail(const F &fkey) {
  if ((isPrimaryRank()) || (!gmode)) {
    std::cout << "DataGetHandlerMF could not find requested file key:"
              << std::endl;
    std::cout << " File stub: " << finfo.getFileStub() << std::endl;
    XMLHandler xmlout;
    fkey.output(xmlout);
...
void DataGetHandlerMF<H, F, R, D>::clearData() {
  for (typename StorageMapType::iterator it = m_storage.begin();
       it != m_storage.end(); it++)
    delete it->second;
  m_storage.clear();
  for (typename FileMapType::iterator it = fileMap.begin(); it != fileMap.end();
       it++)
    close(it->second);
}
...
void DataGetHandlerMF<H, F, R, D>::outputKeys(XMLHandler &xmlout) {
  ...
  for (typename FileMapType::iterator it = fileMap.begin(); it != fileMap.end();
    ...
    for (typename std::set<R>::const_iterator kt = keys.begin();
         kt != keys.end(); kt++) {
...
template <typename H, typename F, typename R, typename D>
DataPutHandlerMF<H, F, R, D>::DataPutHandlerMF(
    H &in_handler, const FileListInfo &in_filelist,
    const std::string &filetype_id, const std::string &header_tag,
    bool global_mode, bool use_checksums, int striping_factor,
    int striping_unit)
    : finfo(in_filelist), handler(in_handler), gmode(global_mode),
      checksums(use_checksums), fid(tidyString(filetype_id)),
      current(fileMap.end()), strpfactor(striping_factor),
      strpunit(striping_unit) {
  for (int suffix = finfo.getMinFileNumber();
       suffix <= finfo.getMaxFileNumber(); suffix++) {
...
DataPutHandlerMF<H, F, R, D>::get_file_ptr(const F &fkey) {
  ...
  if (it != fileMap.end()) {
    if (it->second.fptr == 0)
      openUpdate(it->second);
  } else {
    int findex = 0;
    try {
      findex = finfo.getFirstAvailableSuffix(gmode);
    } catch (const std::exception &xp) {
    ...
    if (pr.second == true)
      ...
      fail("DataPutHandlerMF insertion failed on index " +
...
void DataPutHandlerMF<H, F, R, D>::openUpdate(FileMapValue &fmv) {
  ...
  try {
    fmv.fptr = new IOMap<R, D>(gmode);
    std::string header;
    fmv.fptr->openUpdate(filename, fid, header, 'L', 1, 0, checksums,
...
void DataPutHandlerMF<H, F, R, D>::openNew(const F &fkey, FileMapValue &fmv) {
  ...
  try {
    ...
    fmv.fptr->openNew(filename, fid, headerxml.str(), false, 'L', strpfactor,
                      strpunit, checksums, finfo.isModeOverwrite());
  } catch (const std::exception &xp) {
    fail("failure opening file " + filename + " in DataPutHandlerMF");
  }
}
...
void DataPutHandlerMF<H, F, R, D>::fail(const F &fkey, const R &rkey) {
  if ((isPrimaryRank()) || (!gmode)) {
    std::cout << "DataPutHandlerMF could not insert requested record:"
              << std::endl;
    std::cout << " File stub: " << finfo.getFileStub() << std::endl;
    XMLHandler xmlout;
    xmlout.set_root("FileRecordKey");
...
void DataPutHandlerMF<H, F, R, D>::putData(const R &rkey, const D &data) {
  if (current == fileMap.end()) {
    fail("No current file; cannot insert Data");
  }
  if (current->second.fptr == 0)
    openUpdate(current->second);
  try {
    current->second.fptr->put(rkey, data);
  } catch (const std::exception &xp) {
...
template <typename H, typename F, typename R, typename D>
void DataPutHandlerMF<H, F, R, D>::putData(const F &fkey, const R &rkey,
                                           const D &data) {
  open(fkey);
  try {
    current->second.fptr->put(rkey, data);
...
template <typename H, typename F, typename R, typename D>
bool DataPutHandlerMF<H, F, R, D>::queryData(const F &fkey, const R &rkey) {
  typename FileMapType::iterator it = fileMap.find(fkey);
  if (it == fileMap.end())
    return false;
  if (it->second.fptr == 0)
...
bool DataPutHandlerMF<H, F, R, D>::queryData(const R &rkey) {
  if (current == fileMap.end()) {
    fail("No current file; cannot query Data");
  }
  if (current->second.fptr == 0)
    openUpdate(current->second);
...
void DataPutHandlerMF<H, F, R, D>::merge(const FileListInfo &infiles,
  ...
  for (typename std::set<F>::const_iterator ft = fkeys.begin();
       ft != fkeys.end(); ft++) {
    open(*ft);
    ...
    for (typename std::set<R>::const_iterator rt = rkeys.begin();
         rt != rkeys.end(); rt++) {
...
template <typename H, typename F, typename R, typename D>
void DataPutHandlerMF<H, F, R, D>::merge(
    const std::vector<FileListInfo> &infiles, const std::string &header_tag) {
  for (unsigned int k = 0; k < infiles.size(); k++) {
    printLaph(make_strf("merging file list k = %\n", k));
    std::string instub = infiles[k].getFileStub();
    int nfkeys = 0;
    for (int suffix = infiles[k].getMinFileNumber();
         suffix <= infiles[k].getMaxFileNumber(); suffix++) {
      ...
      for (typename std::set<F>::const_iterator ft = fkeys.begin();
        ...
        open(*ft);
        std::set<R> rkeys = inmerge.getKeys(*ft);
        printLaph(make_strf(" file list %d:  from suffix %d into merged suffix "
                            "%d: number of records added = %d\n",
...
class DataGetHandlerMFNOM : public DataGetHandlerBaseMF<H, F, R, D> {
  ...
  struct MemMapValue {
    ...
    MemMapValue(int in_suff, std::map<R, D> *inptr)
        : suffix(in_suff), mptr(inptr) {}
    ~MemMapValue() {}
  };
...
template <typename H, typename F, typename R, typename D>
DataGetHandlerMFNOM<H, F, R, D>::DataGetHandlerMFNOM(
    H &in_handler, const FileListInfo &in_filelist,
    const std::string &filetype_id, const std::string &header_tag)
  ...
  for (int suffix = finfo.getMinFileNumber();
    ...
    if (NamedObjMap::query(filename)) {
      ...
      try {
        ...
        memMap.insert(std::make_pair(
            fkey, MemMapValue(suffix, &(NamedObjMap::getData<std::map<R, D>>(
                                          filename)))));
      } catch (const std::exception &xp) {
        fail("Could not extract FileKey from file " + filename + "\n");
      }
...
template <typename H, typename F, typename R, typename D>
const D &DataGetHandlerMFNOM<H, F, R, D>::getData(const F &fkey,
                                                  const R &rkey) {
  StorageKey skey(fkey, rkey);
  typename StorageMapType::const_iterator dt = m_storage.find(skey);
  if (dt != m_storage.end())
...
void DataGetHandlerMFNOM<H, F, R, D>::fail(const F &fkey, const R &rkey) {
  std::cout << "DataGetHandlerMFNOM could not find requested record:"
            << std::endl;
  std::cout << " File stub: " << finfo.getFileStub() << std::endl;
  XMLHandler xmlout;
  xmlout.set_root("FileRecordKey");
...
void DataGetHandlerMFNOM<H, F, R, D>::fail(const F &fkey) {
  std::cout << "DataGetHandlerMFNOM could not find requested file key:"
            << std::endl;
  std::cout << " File stub: " << finfo.getFileStub() << std::endl;
  XMLHandler xmlout;
  fkey.output(xmlout);
...
void DataGetHandlerMFNOM<H, F, R, D>::clearData() {
  m_storage.clear();
  for (typename MemMapType::iterator it = memMap.begin(); it != memMap.end();
       it++)
    close(it->second);
}
...
template <typename H, typename F, typename R, typename D>
DataPutHandlerMFNOM<H, F, R, D>::DataPutHandlerMFNOM(
    H &in_handler, const FileListInfo &in_filelist,
    const std::string &filetype_id, const std::string &header_tag)
    : finfo(in_filelist), handler(in_handler), fid(tidyString(filetype_id)),
      current(memMap.end()) {
...
DataPutHandlerMFNOM<H, F, R, D>::get_mem_ptr(const F &fkey) {
  ...
  if (it != memMap.end()) {
    ...
    int findex = 0;
    try {
      findex = finfo.getFirstAvailableSuffixNOM();
    } catch (const std::exception &xp) {
      fail(std::string("could not create file...no more suffix indices") +
           " available...execution aborted...\n");
    }
    ...
    if (pr.second == true)
      ...
      fail("DataPutHandlerMFNOM insertion failed on index " +
           int_to_string(findex));
    }
    openNew(fkey, it->second);
  }
...
void DataPutHandlerMFNOM<H, F, R, D>::fail(const F &fkey, const R &rkey) {
  std::cout << "DataPutHandlerMFNOM could not insert requested record:"
            << std::endl;
  std::cout << " File stub: " << finfo.getFileStub() << std::endl;
  XMLHandler xmlout;
  xmlout.set_root("FileRecordKey");
...
void DataPutHandlerMFNOM<H, F, R, D>::putData(const R &rkey, const D &data) {
  if (current == memMap.end()) {
    fail("No current file; cannot insert Data");
  }
  if (current->second.mptr == 0)
    openUpdate(current->second);
  try {
    if (finfo.isModeOverwrite())
      ...
      fail("Cannot insert Data: record already exists and overwrite not "
           "specified");
    }
  } catch (const std::exception &xp) {
    fail(current->first, rkey);
...
template <typename H, typename F, typename R, typename D>
void DataPutHandlerMFNOM<H, F, R, D>::putData(const F &fkey, const R &rkey,
                                              const D &data) {
  open(fkey);
  try {
    if (finfo.isModeOverwrite())
...
template <typename H, typename F, typename R, typename D>
bool DataPutHandlerMFNOM<H, F, R, D>::queryData(const F &fkey, const R &rkey) {
  typename MemMapType::iterator it = memMap.find(fkey);
  if (it == memMap.end())
    return false;
  if (it->second.mptr == 0)
...
void DataGetHandlerSF<H, R, D>::clearData() {
  for (typename std::map<R, D *>::iterator it = m_storage.begin();
       it != m_storage.end(); it++)
    delete it->second;
  m_storage.clear();
}
...
void DataGetHandlerSF<H, R, D>::fail(const R &rkey) {
  if ((isPrimaryRank()) || (iomptr->isLocal())) {
    std::cout << "DataGetHandlerSF could not find requested record:"
              << std::endl;
...
template <typename H, typename R, typename D>
DataPutHandlerSF<H, R, D>::DataPutHandlerSF(
    H &in_handler, const std::string &file_name, const std::string &filetype_id,
    bool overwrite, bool global_mode, bool use_checksums, int striping_factor,
    int striping_unit)
    : handler(in_handler) {
  XMLHandler headerxml;
  ...
  try {
    ...
    iomptr->openUpdate(file_name, filetype_id, header, 'L', striping_factor,
                       striping_unit, use_checksums, overwrite);
    if (!(iomptr->isNewFile())) {
      XMLHandler xmlr;
...
void DataPutHandlerSF<H, R, D>::fail(const R &rkey) {
  if ((isPrimaryRank()) || (iomptr->isLocal())) {
    std::cout << "DataPutHandlerSF could not insert requested record:"
              << std::endl;
    std::cout << " File name: " << iomptr->getFileName() << std::endl;
    XMLHandler xmlout;
    xmlout.set_root("RecordKey");
...
template <typename H, typename R, typename D>
DataGetHandlerSFNOM<H, R, D>::DataGetHandlerSFNOM(
    H &in_handler, const std::string &filename, const std::string &filetype_id)
...
void DataGetHandlerSFNOM<H, R, D>::fail(const R &rkey) {
  std::cout << "DataGetHandlerSFNOM could not find requested record:"
            << std::endl;
...
template <typename H, typename R, typename D>
DataPutHandlerSFNOM<H, R, D>::DataPutHandlerSFNOM(
    H &in_handler, const std::string &file_name, const std::string &filetype_id,
    bool overwrite)
...
void DataPutHandlerSFNOM<H, R, D>::fail(const R &rkey) {
  std::cout << "DataPutHandlerSFNOM could not insert requested record:"
            << std::endl;
  std::cout << " File name: " << getFileName() << std::endl;
  XMLHandler xmlout;
  xmlout.set_root("RecordKey");
...
template <typename H, typename F, typename R, typename D>
DataGetHandlerMFO<H, F, R, D>::DataGetHandlerMFO(
    H &in_handler, const FileListInfo &in_filelist,
    const std::string &filetype_id, const std::string &header_tag,
    bool global_mode, bool use_checksums) {
  std::string stub = in_filelist.getFileStub();
  if (stub.find("NOM_") != 0) {
    base_ptr = new DataGetHandlerMF<H, F, R, D>(in_handler, in_filelist,
                                                filetype_id, header_tag,
                                                global_mode, use_checksums);
  } else {
    if (!global_mode) {
      errorLaph(
          "Data to/from NamedObjMap in local mode not currently supported");
    }
    stub.erase(0, 4);
    base_ptr = new DataGetHandlerMFNOM<H, F, R, D>(
        in_handler,
        FileListInfo(stub, in_filelist.getMinFileNumber(),
                     in_filelist.getMaxFileNumber(),
...
public:
  ...
  bool queryData(const R &rkey) {
    return base_ptr->queryData(rkey);
  } // query in current open file
  bool queryFile(const F &fkey) { return base_ptr->queryFile(fkey); }
  void getFileMap(XMLHandler &xmlout) const { base_ptr->getFileMap(xmlout); }
  std::map<int, F> getSuffixMap() const { return base_ptr->getSuffixMap(); }
  std::set<F> getFileKeys() const { return base_ptr->getFileKeys(); }
...
template <typename H, typename F, typename R, typename D>
DataPutHandlerMFO<H, F, R, D>::DataPutHandlerMFO(
    H &in_handler, const FileListInfo &in_filelist,
    const std::string &filetype_id, const std::string &header_tag,
    bool global_mode, bool use_checksums, int striping_factor,
    int striping_unit) {
  std::string stub = in_filelist.getFileStub();
  if (stub.find("NOM_") != 0) {
    base_ptr = new DataPutHandlerMF<H, F, R, D>(
        in_handler, in_filelist, filetype_id, header_tag, global_mode,
        use_checksums, striping_factor, striping_unit);
  } else {
    if (!global_mode) {
      errorLaph(
          "Data to/from NamedObjMap in local mode not currently supported");
    ...
    stub.erase(0, 4);
    base_ptr = new DataPutHandlerMFNOM<H, F, R, D>(
        in_handler,
        FileListInfo(stub, in_filelist.getMinFileNumber(),
                     in_filelist.getMaxFileNumber(),
                     in_filelist.isModeOverwrite()),
...
template <typename H, typename R, typename D> class DataGetHandlerSFO {
...
template <typename H, typename R, typename D>
DataGetHandlerSFO<H, R, D>::DataGetHandlerSFO(H &in_handler,
                                              const std::string &filename,
                                              const std::string &filetype_id,
                                              bool global_mode,
                                              bool use_checksums) {
  ...
  if (infile.find("NOM_") != 0) {
    ...
    infile.erase(0, 4);
    base_ptr =
        new DataGetHandlerSFNOM<H, R, D>(in_handler, infile, filetype_id);
  }
}

template <typename H, typename R, typename D> class DataPutHandlerSFO {
...
template <typename H, typename R, typename D>
DataPutHandlerSFO<H, R, D>::DataPutHandlerSFO(
    H &in_handler, const std::string &file_name, const std::string &filetype_id,
    bool overwrite, bool global_mode, bool use_checksums, int striping_factor,
    int striping_unit) {
  std::string infile(tidyString(file_name));
  if (infile.find("NOM_") != 0) {
    base_ptr = new DataPutHandlerSF<H, R, D>(
        in_handler, infile, filetype_id, overwrite, global_mode, use_checksums,
    ...
    infile.erase(0, 4);
    base_ptr = new DataPutHandlerSFNOM<H, R, D>(in_handler, infile, filetype_id,
                                                overwrite);
  }
}

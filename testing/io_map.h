...
namespace LaphEnv {
 ...
 // *    (2) a numbytes(ioh,K) function, where ioh is an IOHandlerFM, must be       *
...
template <typename T>
void multi_write(IOHandlerFM& ioh, const std::vector<T>& output)
{
 int n=output.size();
 if (n<1) return;
 int nint=T::numints();
...
template <typename T>
void multi_read(IOHandlerFM& ioh, std::vector<T>& input, int n)
{
 input.clear();
 if (n<1) return;
 input.reserve(n); 
...
class UIntKey
{
 ...
 public:
   ...
   std::string output() const
   {XMLHandler xmlo; output(xmlo); return xmlo.output();}
...
inline size_t numbytes(IOHandlerFM& ioh, const UIntKey& uikey)
{
...
class IOMap
{
     ...
     typedef IOHandlerFM::pos_type pos_type; 
     typedef IOHandlerFM::off_type off_type; 
     typedef unsigned long long  w_pos;
    
     mutable IOHandlerFM ioh;
  ...
  public:
    ...
    typedef IOHandlerFM::OpenMode OpenMode;
                              ...
                              std::string& header, bool turn_on_checksum)
{
 ...
 OpenMode mode=IOHandlerFM::ReadOnly;
...
{
 check_for_failure(ioh.isOpen(),"Please do not open an already open IOMap!");
 OpenMode mode=(fail_if_exists) ? IOHandlerFM::ReadWriteFailIfExists
                                : IOHandlerFM::ReadWriteEraseIfExists;
...
{
 check_for_failure(ioh.isOpen(),"Please do not open an already open IOMap!");
 OpenMode mode=IOHandlerFM::ReadWriteUpdateIfExists;
 initialize(filename,mode,filetype_id,endianness,striping_factor,striping_unit,
                            ...
                            bool read_header, bool overwrites)
{
 ...
 if (ioh.isNewFile()){                // this is a new file
    ...
    if (read_header) ioh.read(header,isGlobal());
 ...
 if (verbose2){
    ...
    else std::cout << "   IOMap is local on rank "<<LayoutInfo::getMyRank()<<std::endl;

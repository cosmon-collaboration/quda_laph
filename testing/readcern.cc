...
void readCERN(vector<LattFieldDP>& U, const std::string& cfg_file)
{
 if ((sizeof(int)!=4)||(sizeof(double)!=8)){
   errorCuda("CERN files contain 4-byte ints, 8-byte doubles\n");}
 if (QDP::Nc!=3){
 ...
 IOHandlerFM GaugeIO;
 GaugeIO.read_only=true;
 GaugeIO.m_filename=tidyString(cfg_file);
 if (GaugeIO.m_filename.empty()){
     GaugeIO.check_for_failure(IOHandlerFM::IO_ERR_NO_SUCH_FILE,"Empty file name");}
 bool exists=fileExists();
 if (!exists){
    GaugeIO.check_for_failure(IOHandlerFM::IO_ERR_NO_SUCH_FILE,
                "Failure during ReadOnly open: file does not exist");}
 IOHandlerFM::openmode_type access_mode=IOHandlerFM::IO_MODE_RDONLY;
 GaugeIO.is_new_file=false;
 GaugeIO.endian_format='L'; 
     // now open the file for reading
 GaugeIO.file_open(access_mode,"Could not open existing file: "+GaugeIO.m_filename);
 GaugeIO.openflag=true;

 vector<int> Ncern(4);
 double avgplaq;
 GaugeIO.read(Ncern[0]);
 GaugeIO.read(Ncern[1]);
 GaugeIO.read(Ncern[2]);
 GaugeIO.read(Ncern[3]);
 GaugeIO.read(avgplaq);
 ...
 const vector<int>& comm_coords=LayoutInfo::getMyCommCoords();
 const vector<int>& gsizes=LayoutInfo::getLattSizes();
 const vector<int>& lsizes=LayoutInfo::getRankLattSizes();
 const int& locnsites=LayoutInfo::getRankLatticeNumSites();

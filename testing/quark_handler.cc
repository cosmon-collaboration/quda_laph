...
void QuarkHandler::setUpPreconditioning(QudaInvertParam& invParam)
{
 if (invertPtr->getName()=="GCR_MULTIGRID"){
    if (invertPtr->QudaMGInfoPtr){
       ...
       preconditioner=newMultigridQuda(&invertPtr->QudaMGInfoPtr->mg_param); printLaph("done");
...
{   
 ...
 bool fermbchack=(!gaugeHandler->isDataOnDevice())&&(qactionPtr->isFermionTimeBCAntiPeriodic());
 if (fermbchack){
...
//     logQuda(QUDA_VERBOSE,"Fmunu double number %d is %f\n",k,z);}
 ...
 if (quda_inv_param.dslash_type == QUDA_CLOVER_WILSON_DSLASH){    // NEED CHECK HERE than clover is invertible
...
//string fileout("/latticeQCD/raid4/laph/cls21_A653_r000/quark_sinks/1/quda_source_dil_");
//fileout+=make_string(dil);
//printFieldToFile(ferm_srcGR,string("source for dil ")+make_string(dil),fileout);
...
//convertSpinBasis(ferm_snkGR,sinkBatchData[iSinkBatch],"DP_to_GR");
//fileout="/latticeQCD/raid4/laph/cls21_A653_r000/quark_sinks/1/quda_sink_dil_";
//fileout+=make_string(dil);
//printFieldToFile(ferm_snkGR,"SinkGR",fileout);

/*  THIS IS HOW QUDA APPLIES DSLASH??
...
*/
    ...
    LattField sol_DGR(FieldSiteType::ColorSpinVector);
    LattField check_DGR(FieldSiteType::ColorSpinVector);
    convertSpinBasis(sol_DGR,sinkBatchData[iSinkBatch],"DP_to_GR");
    applyCloverDirac(check_DGR,sol_DGR,gaugeHandler->getData(),*uPtr,*qactionPtr);
    convertSpinBasis(sol_check,check_DGR,"GR_to_DP");
    compare_fields(sol_check,ferm_src);
 ...
 //      Array<dcmplx> qudaRes(nSinks, nEigs, Textent, Nspin);
       ...
       laphSinkProject(qudaResPtr, sinks_ptr, nSinks, nSinksBatch, evs_ptr, nEigs, 
                       nEigQudaBatch, &quda_inv_param, LayoutInfo::getRankLattSizes().data());
...
// check projections
       ...
       //Array<dcmplx> slowRes(nSinks, nEigs, Textent, Nspin);
       //laphSinkProjectSlow(slowRes, sinks_ptr, nSinks, evs_ptr, nEigs);

// for (int q=0;q<nSinks;++q)
// for (int v=0;v<nEigs;++v)
// for (int t=0;t<Textent;++t)
// for (int s=0;s<FieldNspin;++s){
//    printLaph(make_strf("res(%d,%d,%d,%d)=(%f,%f) compare (%f,%f)\n",q,v,t,s,real(qudaRes(s,t,v,q)),imag(qudaRes(s,t,v,q)),
//                       real(slowRes(q,v,t,s)),imag(slowRes(q,v,t,s))));}
...
//                   quark_sink(iEv) = soln_rescale*slowRes(iSink, iEv, t, iSpin);}

...
bool GaugeCERNConfigReader::read(vector<LattField>& u, const std::string& in_cfg_file)
{
 ...
 int NZ = LayoutInfo::getLattSizes()[2];   // these should all be even
...
bool GaugeCERNConfigReader::read(vector<LattField>& u, const std::string& in_cfg_file)
{
 ...
 for (int k=0;k<LayoutInfo::Ndim;++k){
    global_sizes[k]=LayoutInfo::getLattSizes()[(LayoutInfo::Ndim+2-k)% LayoutInfo::Ndim]; 
    local_sizes[k]=LayoutInfo::getRankLattSizes()[(LayoutInfo::Ndim+2-k)% LayoutInfo::Ndim];

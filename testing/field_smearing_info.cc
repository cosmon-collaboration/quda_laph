...
void GluonSmearingInfo::setQudaGaugeSmearParam(QudaGaugeSmearParam &gauge_smear_param) const
{
 ...
 gauge_smear_param.meas_interval = linkIterations;
...
void QuarkSmearingInfo::extract_info_from_reader(XMLHandler& xml_in)
{
 ...
 if ((laphNumEigvecs<1)||(laphSigma<=0)){
    throw(std::invalid_argument("error: invalid smearing scheme parameters in QuarkSmearingInfo"));}
}

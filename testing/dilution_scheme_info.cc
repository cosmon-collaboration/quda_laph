                                ...
                                int time_dil_type)
{
 try{
    ...
    if ((spin_dil_type>2)||(spin_dil_type<-2))
       throw(std::invalid_argument("spin dilution type must have value -2, 0, 1, 2"));
    }
...
void DilutionSchemeInfo::checkEqual(const DilutionSchemeInfo& in) const
{
 if ((spinDilutionType!=in.spinDilutionType)
   ...
   ||(timeDilutionType!=in.timeDilutionType)){
    ...
    throw(std::invalid_argument("DilutionSchemeInfo does not checkEqual...abort"));}
}
...
bool DilutionSchemeInfo::canUndilute(const DilutionSchemeInfo& newscheme) const
{
                                ...
                                int& DilType)
{
 ...
 try{
    ...
    if (tmp=="none"){
       ...
       else throw(std::runtime_error("invalid number of block projectors"));
       }
       ...
       if (xmlr.count("NumberProjectors")==1){
          xmlread(xmlr,"NumberProjectors",nproj,"DilutionSchemeInfo");}
       if (nproj>1) DilType=-nproj;
       else throw(std::runtime_error("invalid number of interlace projectors"));
       }
    else{
       throw(std::runtime_error("invalid read"));}
    }
 catch(const std::exception& errstr){
       std::cerr << "invalid DilutionSchemeInfo read from XML"<<errstr.what()<<endl;
       throw(std::runtime_error("error"));}
}


void DilutionSchemeInfo::dil_out(XMLHandler& xmlout,
...
{
 if (dilorig==1) return true;
 if (dilnew==1) return false;
 if (dilorig==0) return (dilnew==0);
 if ((dilorig>=2)&&(dilnew<0)) return false;
 if ((dilorig<=-2)&&(dilnew>0)) return false;
 int dorig=(dilorig>=2)?dilorig:-dilorig;
 int dnew=(dilnew>=2)?dilnew:-dilnew;
 if (dorig>=2){
    if (dnew==0) return true;
    else if (dnew==1) return false;
    if ((dorig%dnew)==0) return true;
    return false;}
 return false;
}

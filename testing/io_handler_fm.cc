...
const int IOHandlerFM::ID_string_length = 32;
const IOHandlerFM::pos_type IOHandlerFM::data_start_pos = 1+IOHandlerFM::ID_string_length;
const int IOHandlerFM::Nd=4;

ByteHandler IOHandlerFM::m_bytehandler;
...
void IOHandlerFM::delete_file()
{
 check_for_failure(remove(m_filename.c_str()),"Failure deleting file");
}

void IOHandlerFM::file_open(IOHandlerFM::openmode_type access_mode, 
...
void IOHandlerFM::file_close()
{
 fh.close();
 check_for_failure(!fh, "Failure during close");
}

const int IOHandlerFM::IO_ERR_NO_SUCH_FILE= 1;
const int IOHandlerFM::IO_ERR_ACCESS=       1;
const int IOHandlerFM::IO_ERR_OTHER=        1;
const int IOHandlerFM::IO_SUCCESS=          0;

const IOHandlerFM::openmode_type IOHandlerFM::IO_MODE_RDONLY= std::ios::in;
const IOHandlerFM::openmode_type IOHandlerFM::IO_MODE_RDWR=   std::ios::in | std::ios::out;
const IOHandlerFM::openmode_type IOHandlerFM::IO_MODE_CREATE= std::ios::trunc;
const IOHandlerFM::openmode_type IOHandlerFM::IO_MODE_EXCL=   std::ios::trunc;

const IOHandlerFM::whence_type IOHandlerFM::IO_SEEK_BEG  = std::ios::beg;
const IOHandlerFM::whence_type IOHandlerFM::IO_SEEK_CUR  = std::ios::cur;
const IOHandlerFM::whence_type IOHandlerFM::IO_SEEK_END  = std::ios::end;


void IOHandlerFM::check_for_failure(int errcode, const std::string& mesg)
{
 if (errcode==IOHandlerFM::IO_SUCCESS) return;
 errorLaph(make_strf("IOHandlerFM error with file %s:\n %s\n",m_filename,mesg));
}

void IOHandlerFM::set_striping(int striping_factor, int striping_unit)
{
}


void IOHandlerFM::file_seek(off_type offset, whence_type whence)
{
 fh.seekg(offset, whence);
 check_for_failure(fh.fail(),"Failure during seek");
}

IOHandlerFM::pos_type IOHandlerFM::file_tell()
{
...
void IOHandlerFM::printFileHints()
{
 printLaph(make_str("Serial mode: no IOHandlerFM file hints\n"));
}


void IOHandlerFM::writeCommon(const char *data, size_t nbytes)
{
...
void IOHandlerFM::writeDistributed(const char *data, const DistArrayViewInfo& dinfo,
                                   bool datashift)
{
...
void IOHandlerFM::readCommon(char *data, size_t nbytes, bool broadcast)
{
 pos_type currdisp=fh.tellg();
 fh.read(data,nbytes);
...
void IOHandlerFM::readDistributed(char *data, const DistArrayViewInfo& dinfo,
                                  bool datashift)
{
 char* dataptr=data;
 if (datashift) dataptr+=dinfo.getElementOffsetThisRank()*dinfo.getBytesPerElement();
...
void IOHandlerFM::compute_lattice_checksum(char* data, size_t bytes_per_site,
...
const int IOHandlerFM::IO_ERR_NO_SUCH_FILE= MPI_ERR_NO_SUCH_FILE;
const int IOHandlerFM::IO_ERR_ACCESS=       MPI_ERR_ACCESS;
const int IOHandlerFM::IO_ERR_OTHER=        MPI_ERR_OTHER;
const int IOHandlerFM::IO_SUCCESS=          MPI_SUCCESS;

const IOHandlerFM::openmode_type IOHandlerFM::IO_MODE_RDONLY= MPI_MODE_RDONLY;
const IOHandlerFM::openmode_type IOHandlerFM::IO_MODE_RDWR=   MPI_MODE_RDWR;
const IOHandlerFM::openmode_type IOHandlerFM::IO_MODE_CREATE= MPI_MODE_CREATE;
const IOHandlerFM::openmode_type IOHandlerFM::IO_MODE_EXCL=   MPI_MODE_EXCL;

const IOHandlerFM::whence_type IOHandlerFM::IO_SEEK_BEG  = MPI_SEEK_SET;
const IOHandlerFM::whence_type IOHandlerFM::IO_SEEK_CUR  = MPI_SEEK_CUR;
const IOHandlerFM::whence_type IOHandlerFM::IO_SEEK_END  = MPI_SEEK_END;
...
void IOHandlerFM::check_for_failure(int errcode, const std::string& mesg)
{
 ...
 if ((isPrimaryRank())||(!global_mode))
#endif
    {printf("IOHandlerFM error with file %s:\n  %s\n%s\n",m_filename.c_str(),
...
void IOHandlerFM::file_seek(off_type offset, whence_type whence)
{
 if (whence==IOHandlerFM::IO_SEEK_END){   // MPI_SEEK_END does not seem to work properly
    MPI_Offset fsize;
    check_for_failure(MPI_File_get_size(fh,&fsize),"Failure during seek");
    check_for_failure(MPI_File_seek(fh,fsize+offset,IOHandlerFM::IO_SEEK_BEG),
                     "Failure during seek");}
 else{
                                           ...
                                           size_t lexicostart, size_t nsites)
{
 ...
 uint rank=0;
 uint count=0;
 const uint xinc = LayoutInfo::getRankLattSizes()[0];
...
IOHandlerFM::IOHandlerFM(bool global) : read_only(true), openflag(false), read_mode(true),
...
IOHandlerFM::IOHandlerFM(const std::string& filename, OpenMode mode,
...
IOHandlerFM::IOHandlerFM(const std::string& filename, std::ios_base::openmode mode,
                         const std::string& filetype_id, char endianness,
                         int striping_factor, int striping_unit,
                         bool turn_on_checksum, bool global) : global_mode(global), lattinfo(1),
                         lattcmplxbytes(LattField::get_cpu_prec_bytes())
...
void IOHandlerFM::open(const std::string& filename, IOHandlerFM::OpenMode mode,
                       const std::string& filetype_id, char endianness,
                       int striping_factor, int striping_unit,
                       bool turn_on_checksum)
{
 ...
 if ((mode==ReadOnly)&&(!exists))
    ...
    exists=false;}
 if (exists){
    IOHandlerFM::openmode_type access=(mode==ReadOnly) ? 
...
void IOHandlerFM::open(const std::string& filename, std::ios_base::openmode mode,
                       const std::string& filetype_id, char endianness,
                       int striping_factor, int striping_unit,
                       bool turn_on_checksum)
{
 IOHandlerFM::OpenMode iomode;
...
void IOHandlerFM::openReadOnly(const std::string& filename, 
...
void IOHandlerFM::openNew(const std::string& filename, bool fail_if_exists,
                          const std::string& filetype_id, char endianness,
                          int striping_factor, int striping_unit,
                          bool turn_on_checksum)
{
...
void IOHandlerFM::openUpdate(const std::string& filename,
                             const std::string& filetype_id, char endianness,
                             int striping_factor, int striping_unit,
                             bool turn_on_checksum)
{
...
void IOHandlerFM::open_existing_file(const std::string& filetype_id,
                                     IOHandlerFM::openmode_type access_mode)
{
...
void IOHandlerFM::open_new_file(const std::string& filetype_id,
                                char endianness,
                                int striping_factor, int striping_unit)
{
 ...
 IOHandlerFM::openmode_type amode = IO_MODE_RDWR | IO_MODE_CREATE;
...
bool IOHandlerFM::peekString(std::string& stringvalue, size_t byte_offset,
...
bool IOHandlerFM::peeker(std::string& stringvalue, size_t byte_offset,
...
void IOHandlerFM::seekFromStart(off_type offset)
{
 file_seek(offset+data_start_pos,IOHandlerFM::IO_SEEK_BEG);
 checksum=0;
}

void IOHandlerFM::seekFromCurr(off_type offset)
{ 
 file_seek(offset,IOHandlerFM::IO_SEEK_CUR);
 checksum=0;
}
 
void IOHandlerFM::seekFromEnd(off_type offset)
{ 
 file_seek(offset,IOHandlerFM::IO_SEEK_END);
 checksum=0;
}

void IOHandlerFM::seek(pos_type offset)
{ 
 file_seek(offset+data_start_pos,IOHandlerFM::IO_SEEK_BEG);
 checksum=0;
}

void IOHandlerFM::seekBegin(off_type offset)
{ 
 file_seek(offset+data_start_pos,IOHandlerFM::IO_SEEK_BEG);
 checksum=0;
}

void IOHandlerFM::seekRelative(off_type offset)
{ 
 file_seek(offset,IOHandlerFM::IO_SEEK_CUR);
 checksum=0;
}

void IOHandlerFM::seekEnd(off_type offset)
{ 
 file_seek(offset,IOHandlerFM::IO_SEEK_END);
 checksum=0;
}

void IOHandlerFM::rewind()
{ 
 file_seek(data_start_pos,IOHandlerFM::IO_SEEK_BEG);
 checksum=0;
}
...
IOHandlerFM::pos_type IOHandlerFM::currentPosition()
{
 return file_tell()-data_start_pos;
}

IOHandlerFM::pos_type IOHandlerFM::tell()
{
 return file_tell()-data_start_pos;
}


void IOHandlerFM::turnOnChecksum()
{
...
void IOHandlerFM::turnOffChecksum()
{
 checksum_on=false;
}

void IOHandlerFM::resetChecksum()
{
 checksum=0;
}

ByteHandler::n_uint32_t IOHandlerFM::getChecksum()
{
...
void IOHandlerFM::printFileID()
{
 char endian;
 string ID_string;
 pos_type curr=file_tell();
 readIDstring(endian,ID_string);
 file_seek(curr,IOHandlerFM::IO_SEEK_BEG);
 if ((isPrimaryRank())||(!global_mode)){
...
void IOHandlerFM::writeIDstring(const std::string& ID_string)
{
 check_for_failure(int(ID_string.length())>ID_string_length,
                  "IOHandlerFM file ID string too long: cannot exceed "
 ...
 if (nblanks>0)
    buf+=string(nblanks,' ');
 file_seek(0,IOHandlerFM::IO_SEEK_BEG);
...
void IOHandlerFM::readIDstring(char &endianness, std::string& ID_string)
{
 if ((isPrimaryRank())||(!global_mode)){
...
string IOHandlerFM::int_to_string(int intval)
{ 
...
string IOHandlerFM::tidyString(const string& str)   
{
 string tmp;
...
void IOHandlerFM::write_common(const char* output, size_t elementbytes, size_t nelements)
{
 if (!openflag)
    check_for_failure(IO_ERR_OTHER,"Write failure--no open file");
 if (read_only)
...
void IOHandlerFM::read_common(char* input, size_t elementbytes, size_t nelements)
{
 if (!openflag)
    check_for_failure(IO_ERR_OTHER,"Read failure--no open file");
 if (!read_mode){
...
void IOHandlerFM::write_lattice(const char* output, size_t bytes_per_word, 
...
void IOHandlerFM::read_lattice(char* input, size_t bytes_per_word, 
                               const DistArrayViewInfo& dinfo)
{
 if (!openflag)
    check_for_failure(IO_ERR_OTHER,"Read failure--no open file");
...
void IOHandlerFM::write(const LattField& output)
{
 ...
 for (int k=0;k<LayoutInfo::Ndim;k++)
    sz[k+1]=LayoutInfo::getLattSizes()[k];
...
void IOHandlerFM::write(const std::vector<LattField>& output)
{
 int n=output.size();
 write(n);
 if (n==0) return;
...
void IOHandlerFM::read(LattField& input)
{
 unsigned int sz[LayoutInfo::Ndim+2];
 multi_read(sz,LayoutInfo::Ndim+2);
 bool errflag=(sz[0]!=(unsigned int)(LayoutInfo::Ndim));
 for (int k=0;k<LayoutInfo::Ndim;k++)
    errflag=errflag || (sz[k+1]!=(unsigned int)(LayoutInfo::getLattSizes()[k]));
...
void IOHandlerFM::read(std::vector<LattField>& input)
{
 int n;
 read(n);
 if (n==0) return;
...
size_t IOHandlerFM::numbytes(const LattField& data)
{
 return (LayoutInfo::Ndim+2)*sizeof(int)+lattcmplxbytes*data.elemsPerSite()
         *LayoutInfo::getLatticeNumSites();
}

size_t IOHandlerFM::numbytes(const std::vector<LattField>& data)
{
 if (data.size()>0){
...
void IOHandlerFM::write(const std::string& output)
{
 int n=output.length();
...
void IOHandlerFM::read(std::string& input, bool broadcast)
{
 int n;
 ...
 check_for_failure((str==0),"IOHandlerFM::read---unable to allocate memory");
...
size_t numbytes(IOHandlerFM& ioh, const std::string& output)
{
...
{
 setup_full(LayoutInfo::getLattSizes(),LayoutInfo::getCommNumPartitions(),LayoutInfo::getMyCommCoords());
...
DistArrayViewInfo::DistArrayViewInfo(IOH_int bytes_per_site)
{
 setup_full(LayoutInfo::getLattSizes());
 set_bytes(bytes_per_site);
} 

...
std::vector<int> LayoutInfo::latt_sizes;

std::vector<int> LayoutInfo::num_partitions;

std::vector<int> LayoutInfo::rank_latt_sizes;
...
void LayoutInfo::init(const XMLHandler& xmlin, const std::vector<int> npartitions, bool echo)
{
 ...
 if (latt_sizes.size()==Ndim){
    throw(std::runtime_error("LayoutInfo::init can only be run ONCE! Don't try it again!"));}
 xmlreadchild(xml_in,"XYZTSizes",latt_sizes,"LayoutInfo");
 if (latt_sizes.size()!=Ndim){
    throw(std::runtime_error("XYZTSizes in invalid"));}
 ...
 for (int k=0;k<Ndim;++k){
    latt_nsites*=latt_sizes[k];
    rank_latt_sizes[k]=latt_sizes[k]/num_partitions[k];
    if ((rank_latt_sizes[k]*num_partitions[k])!=latt_sizes[k]){
       errorLaph("Lattice is not partitioned into identical sublattices");}
    rank_latt_nsites*=rank_latt_sizes[k];}
 if ((rank_latt_nsites%2)==1){
    errorLaph("Number of sites on each MPI rank must be even to accommodate checkerboarding");}
 set_up_comm_map();
 my_start_parity=0;
 for (int k=0;k<Ndim;++k){
    my_start_parity+=my_comm_coords[k]*rank_latt_sizes[k];}
 ...
 if (echo){
    ...
    if (npartitions.size()==Ndim){
       ...
       xml_in.put_sibling("XYZTSizesEachRank",make_string(rank_latt_sizes));}
#endif
                                          ...
                                          std::vector<int>& rank_site_coords)
{
 ...
 for (int dir=0;dir<Ndim;++dir){
    comm_coords[dir]=latt_coords[dir]/rank_latt_sizes[dir];
    rank_site_coords[dir]=latt_coords[dir]-comm_coords[dir]*rank_latt_sizes[dir];}
}
                                              ...
                                              int& rank, int& rank_site_linear_index)
{
 ...
 rank_site_linear_index=rank_site_coords[0]+rank_latt_sizes[0]*(rank_site_coords[1]
                       +rank_latt_sizes[1]*(rank_site_coords[2]+rank_latt_sizes[2]*rank_site_coords[3]));
}
*/
                                              ...
                                              int& rank, int& rank_site_linear_index)
{
 ...
 int start_parity=0;
 for (int k=0;k<Ndim;++k){
    start_parity+=comm_coords[k]*rank_latt_sizes[k];}
 ...
 rank_site_linear_index=get_linear_index(start_parity,
             ...
             rank_latt_sizes[0],rank_latt_sizes[1],rank_latt_sizes[2],rank_latt_nsites);
}
...
int LayoutInfo::getRankFromLexicoLinearIndex(int lexico_lin_index)
{
 ...
 for (int k=0;k<Ndim;++k){
    latt_coords[k]=index % latt_sizes[k];
    index-=latt_coords[k]; index/=latt_sizes[k];}
...
{
 int locNX = LayoutInfo::getRankLattSizes()[0];
 int locNY = LayoutInfo::getRankLattSizes()[1];
 int locNZ = LayoutInfo::getRankLattSizes()[2];
 int locNT = LayoutInfo::getRankLattSizes()[3];
 const char* lex=src;
 char* even=dest;
...
int LayoutInfo::linearSiteIndex_lexico(const vector<int>& loc_coords, const vector<int>& loc_sizes)
{
 return loc_coords[0]+loc_sizes[0]*(loc_coords[1]+loc_sizes[1]*(loc_coords[2]
     ...
     // assumes only that number of sites on each MPI process is even
int LayoutInfo::linearSiteIndex_evenodd(const vector<int>& loc_coords, const vector<int>& loc_sizes,

#ifndef IO_HANDLER_FM_H
#define IO_HANDLER_FM_H
...
namespace LaphEnv {
 ...
 // *********************************************************************************
 // *                                                                               *
 // *       class IOHandlerFM:    parallel random access input/output               *
 // *                                                                               *
 ...
 // *                                                                               *
 // *   An object of class IOHandlerFM must be defined as global or local in the    *
 ...
 // *                                                                               *
 // *      IOHandlerFM io;     <-- default is global mode                           *
 // *      io.open(....);                                                           *
 ...
 // *   NOT stored in the file.  IOMap uses IOHandlerFM, and IOMap maintains        *
 ...
 // *                                                                               *
 // *   For local read/writes, create an IOHandlerFM object on one processor in     *
 // *   local mode:                                                                 *
 // *                                                                               *
 // *      bool global_mode=false;                                                  *
 // *      IOHandlerFM localhandler(global_mode);    <-  local write                *
 // *                                                                               *
 ...
 //  IOHandlerFM class is defined later in this file, after the 
 //  helper class DistArrayViewInfo.
  ...
  /*  DistArrayViewInfo objects contain info about multi-dimensional arrays 
      ...
      used by IOHandlerFM for creating file views for MPI-IO. It supports
...
void TimeSliceOf<T>::setCurrentTime(int time_slice)
{
 ...
 if ((current_time<0)||(current_time>=LayoutInfo::lattSize()[QDP::Nd-1])){
    throw(std::invalid_argument("invalid time in Lattice TimeSliceOf"));}
}
...
*/
 ...
 // ***********************************************************
 // *                                                         *
 // *          Now for the main event:  IOHandlerFM           *
 // *                                                         *
 // ***********************************************************
...
class IOHandlerFM
{
   ...
   int lattcmplxbytes;    // needed to handle precision conversion in IOMap
      ...
      // disallow copying
   IOHandlerFM(const IOHandlerFM&);
   IOHandlerFM(IOHandlerFM&);
   IOHandlerFM& operator=(const IOHandlerFM&);
   IOHandlerFM& operator=(IOHandlerFM&);
 ...
 public:
   ...
   explicit IOHandlerFM(bool global=true);

   IOHandlerFM(const std::string& filename, OpenMode mode=ReadOnly,
   ...
   IOHandlerFM(const std::string& filename, std::ios_base::openmode mode,
               const std::string& filetype_id="", char endianness='N',
               int striping_factor=1, int striping_unit=0,
               bool turn_on_checksum=false, bool global=true);
   ...
   typedef int          openmode_type;
         ...
         //  to an IOHandlerFM::off_type(sizeof(...)) if you wish to use 
...
template <typename T>
void IOHandlerFM::write(const T& output)
{ 
 IOHandlerFM::BasicType dummy(output);  // cause compiler error if T is not a valid basic type
 write_common((const char*)&output, sizeof(T), 1);
}


template <typename T>
void IOHandlerFM::multi_write(const T* output, int n)
{ 
 IOHandlerFM::BasicType dummy(*output);  // cause compiler error if T is not a valid basic type
...
template <typename T>
void IOHandlerFM::write(const std::vector<T>& output)
{
 IOHandlerFM::BasicType dummy(output[0]);  // cause compiler error if T is not a valid basic type
 int n=output.size();
 write(n);
 if (n==0) return;
...
template <typename T>
void IOHandlerFM::write(const Array<T>& output)
{
...
/*
template <typename T>
void IOHandlerFM::writeTimeSlice(const LattField<T>& output, int time_slice)
{
 int sizeT=sizeof(T);
...
template <typename T>
void IOHandlerFM::write(const TimeSliceOf<T>& output)
{
...
template <typename T>
void IOHandlerFM::write(const std::vector<TimeSliceOf<T> >& output)
{
 int n=output.size();
 write(n);
 if (n==0) return;
...
template<typename T>
void write(IOHandlerFM& io, const T& output)   // basic types
 { io.write(output); }

template <typename T>
void write(IOHandlerFM& io, const std::vector<T>& output)
 { io.write(output); }

template <typename T>
void write(IOHandlerFM& io, const Array<T>& output)  // T is basic type
 { io.write(output); }

inline void write(IOHandlerFM& io, const LattField& output)
 { io.write(output); }

inline void write(IOHandlerFM& io, const std::vector<LattField>& output)
 { io.write(output); }

/*
template <typename T>
void write(IOHandlerFM& io, const TimeSliceOf<T>& output)
 { io.write(output); }
  
template <typename T>
void write(IOHandlerFM& io, const std::vector<TimeSliceOf<T> >& output)
 { io.write(output); }
*/
...
template <typename T>
void IOHandlerFM::read(T& input)
{ 
 IOHandlerFM::BasicType dummy(input);  // cause compiler error if T is not a valid basic type
 read_common((char*)&input, sizeof(T), 1);
}


template <typename T>
void IOHandlerFM::multi_read(T* input, int n)
{ 
 IOHandlerFM::BasicType dummy(*input);  // cause compiler error if T is not a valid basic type
...
template <typename T>
void IOHandlerFM::read(std::vector<T>& input)
{
 IOHandlerFM::BasicType dummy(input[0]);  // cause compiler error if T is not a valid basic type
 int n;
 read(n);
 if (n==0) return;
...
template <typename T>
void IOHandlerFM::read(Array<T>& input)
{
 unsigned int n;
 read(n);
 if (n==0) return;
...
/*
template <typename T>
void IOHandlerFM::readTimeSlice(OLattice<T>& input, int time_slice)
{
 ...
 for (int k=0;k<QDP::Nd-1;k++)
    errflag=errflag || (int(sz[k+1])!=LayoutInfo::lattSize()[k]);
...
template <typename T>
void IOHandlerFM::read(TimeSliceOf<T>& input)
{
...
template <typename T>
void IOHandlerFM::read(std::vector<TimeSliceOf<T> >& input)
{
 int n;
 read(n);
...
template<typename T>
void read(IOHandlerFM& io, T& input)   // basic types
 { io.read(input); }

template <typename T>
void read(IOHandlerFM& io, std::vector<T>& input)
 { io.read(input); }

template <typename T>
void read(IOHandlerFM& io, Array<T>& input)  // T is basic type
 { io.read(input); }

inline void read(IOHandlerFM& io, LattField& input)
 { io.read(input); }

inline void read(IOHandlerFM& io, std::vector<LattField>& input)
 { io.read(input); }
/*
template <typename T>
void read(IOHandlerFM& io, std::vector<OLattice<T> >& input)
 { io.read(input); }

template <typename T>
void read(IOHandlerFM& io, TimeSliceOf<T>& input)
 { io.read(input); }
  
template <typename T>
void read(IOHandlerFM& io, std::vector<TimeSliceOf<T> >& input)
 { io.read(input); }
*/
...
template <typename T>
size_t IOHandlerFM::numbytes(const T& data)
{ 
 IOHandlerFM::BasicType dummy(data);  // cause compiler error if T is not a valid basic type
 return sizeof(T);
}

template <typename T>
size_t IOHandlerFM::numbytes(const std::vector<T>& data)
{
 IOHandlerFM::BasicType dummy(data[0]);  // cause compiler error if T is not a valid basic type
 return sizeof(int)+data.size()*sizeof(T);
}

template <typename T>
size_t IOHandlerFM::numbytes(const Array<T>& data)
{
...
template <typename T>
size_t IOHandlerFM::numbytes(const std::vector<OLattice<T> >& data)
{
 return sizeof(int)+data.size()*((QDP::Nd+2)*sizeof(int)+LayoutInfo::vol()*sizeof(T));
}
...
template <typename T>
size_t IOHandlerFM::numbytes(const TimeSliceOf<T>& data)
{
 IOHandlerFM::off_type fullsize=numbytes(data.getData())-(QDP::Nd+2)*sizeof(int);
 fullsize=fullsize/LayoutInfo::lattSize()[QDP::Nd-1];
 return (QDP::Nd+1)*sizeof(int)+fullsize;
}


template <typename T>
size_t IOHandlerFM::numbytes(const std::vector<TimeSliceOf<T> >& data)
{
...
template<typename T>
size_t numbytes(IOHandlerFM& io, const T& data)   // generic routine
 { return data.numbytes(); }

template<> inline size_t numbytes(IOHandlerFM& io, const char& data) 
{ return io.numbytes(data); }

template<> inline size_t numbytes(IOHandlerFM& io, const int& data)  
{ return io.numbytes(data); }

template<> inline size_t numbytes(IOHandlerFM& io, const unsigned int& data) 
{ return io.numbytes(data); }

template<> inline size_t numbytes(IOHandlerFM& io, const short int& data)  
{ return io.numbytes(data); }

template<> inline size_t numbytes(IOHandlerFM& io, const unsigned short int& data) 
{ return io.numbytes(data); }

template<> inline size_t numbytes(IOHandlerFM& io, const long int& data)  
{ return io.numbytes(data); }

template<> inline size_t numbytes(IOHandlerFM& io, const unsigned long int& data) 
{ return io.numbytes(data); }

template<> inline size_t numbytes(IOHandlerFM& io, const long long int& data)  
{ return io.numbytes(data); }

template<> inline size_t numbytes(IOHandlerFM& io, const unsigned long long int& data) 
{ return io.numbytes(data); }

template<> inline size_t numbytes(IOHandlerFM& io, const float& data)  
{ return io.numbytes(data); }

template<> inline size_t numbytes(IOHandlerFM& io, const double& data) 
{ return io.numbytes(data); }

template<> inline size_t numbytes(IOHandlerFM& io, const bool& data)  
{ return io.numbytes(data); }

template<> inline size_t numbytes(IOHandlerFM& io, const cmplx& data) 
{ return io.numbytes(data); }

template<> inline size_t numbytes(IOHandlerFM& io, const fcmplx& data) 
{ return io.numbytes(data); }

template <typename T>
size_t numbytes(IOHandlerFM& io, const std::vector<T>& data)
 { return io.numbytes(data); }

template <typename T>
size_t numbytes(IOHandlerFM& io, const Array<T>& data)
 { return io.numbytes(data); }

inline size_t numbytes(IOHandlerFM& io, const LattField& data)
 { return io.numbytes(data); }

inline size_t numbytes(IOHandlerFM& io, const std::vector<LattField>& data)
 { return io.numbytes(data); }
/*
template <typename T>
size_t numbytes(IOHandlerFM& io, const std::vector<OLattice<T> >& data)
 { return io.numbytes(data); }

template <typename T>
size_t numbytes(IOHandlerFM& io, const TimeSliceOf<T>& data)
 { return io.numbytes(data); }
  
template <typename T>
size_t numbytes(IOHandlerFM& io, const std::vector<TimeSliceOf<T> >& data)
 { return io.numbytes(data); }
*/

...
GluonSmearingHandler::~GluonSmearingHandler() { clear(); }
...
GluonSmearingHandler::getGaugeConfigurationInfo() const {
...
void GluonSmearingHandler::computeSmearedGaugeField() {
  ...
  printLaph(
      "Computation of smeared gauge field commencing in GluonSmearingHandler");
  ...
  bulova.stop();
  printLaph(make_strf("Smearing computation done: time was %g seconds",
  ...
  printLaph(make_strf("Time to write to host and then to file was %g seconds",
                      bulova.getTimeInSeconds()));

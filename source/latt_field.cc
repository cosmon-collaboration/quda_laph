...
LattField& LattField::reset_by_bytes_per_site(int bytes_per_site)
{
 ...
 if      (m_site_bytes==8)  { m_site_elems=1;  m_sitetype=Complex;}
 else if (m_site_bytes==16) { m_site_elems=1;  m_sitetype=Complex;}
 else if (m_site_bytes==24) { m_site_elems=3;  m_sitetype=ColorVector;}
 else if (m_site_bytes==48) { m_site_elems=3;  m_sitetype=ColorVector;}
 else if (m_site_bytes==72) { m_site_elems=9;  m_sitetype=ColorMatrix;}
 else if (m_site_bytes==144){ m_site_elems=9;  m_sitetype=ColorMatrix;}
 else if (m_site_bytes==96) { m_site_elems=12; m_sitetype=ColorSpinVector;}
 else if (m_site_bytes==192){ m_site_elems=12; m_sitetype=ColorSpinVector;}
 else{
    errorLaph(make_str("Invalid resizing of LattField with bytes_per_site = ",bytes_per_site));}
 if (m_site_bytes>0){
    m_data.resize(LayoutInfo::getRankLatticeNumSites()*m_site_bytes);}
 else{
    m_data.clear();}
...
std::vector<char> LattField::getSiteData(const std::vector<int>& latt_coords) const
{
...
LattField& LattField::to_quda_precision()
{
 ...
 else if (cmplx_bytes==size_t(2*cpu_prec_bytes)){  // convert double to single
    vector<char> buffer;
    to_single_precision(buffer);
    m_data=std::move(buffer);
    m_site_bytes/=2;}
 else if ((2*cmplx_bytes)==size_t(cpu_prec_bytes)){  // convert single to double (warn!)
    vector<char> buffer;
...
void LattField::to_single_precision(vector<char>& buffer)
{
 ...
 int n=2*LayoutInfo::getRankLatticeNumSites()*m_site_elems; // number of reals
...
{
 printLaph("WARNING: converting lattice field from single to double precision");
...
void LattField::applyFermionTemporalAntiPeriodic()
{
 ...
 if (LayoutInfo::getMyCommCoords()[Tdir]!=(LayoutInfo::getCommNumPartitions()[Tdir]-1)){
    return;}
 ...
 for (int y=0;y<N[1];++y)
 for (int x=0;x<N[0];++x){
    int sindex=nreal*( (x+N[0]*(y+N[1]*(z+N[2]*t)))/2 + npsites*((start_parity+x+y+z+t)%2));
    if (dp){
       double* ptr=reinterpret_cast<double*>(m_data.data())+sindex;
       for (int s=0;s<nreal;++s,++ptr) *ptr=-(*ptr);}
    else{

...
template<typename T>
class NamedObj : public NamedObjBase 
{
   ...
   virtual void setXMLInfo(XMLHandler& xml) 
   {xml_info=xml.output_current();}
...
class NamedObjMap
{
    ...
    typedef std::map<std::string, NamedObjBase*>::const_iterator nom_const_iterator;
 ...
 public:
    ...
    static void clear() 
    {for (std::map<std::string,NamedObjBase*>::iterator it=the_map.begin();it!=the_map.end();++it){
        delete it->second;}
     the_map.clear();}


    template<typename T>
    static T& insert(const std::string& id) 
    {
     if (the_map.find(id)!=the_map.end()){
        throw(std::runtime_error("Attempt to NamedObjMap::insert an id already in map"));}
     ...
     std::pair<nom_iterator,bool> result=the_map.insert(make_pair(id,newdata));
     if (!result.second){
        throw(std::runtime_error("Attempt to NamedObjMap::insert failed"));}
    ...
    template<typename T>
    static T& insert(const std::string& id, const T& data)
    {
     if (the_map.find(id)!=the_map.end()){
        throw(std::runtime_error("Attempt to NamedObjMap::insert an id already in map"));}
    ...
    template<typename T>
    static T& getData(const std::string& id) 
    {return dynamic_cast<NamedObj<T>&>(get(id)).getData();}


    template<typename T>
    static void getData(const std::string& id, T* &data) 
    ...
    template<typename T>
    static void getDataAndXMLInfo(const std::string& id, T* &data, XMLHandler& xmlinfo) 
    {
    ...
    template<typename T>
    static void getDataAndXMLInfo(const std::string& id, const T* &data, XMLHandler& xmlinfo)
    {
 ...
 private:
    ...
    static NamedObjBase& get(const std::string& id)
    {
     nom_iterator it = the_map.find(id);
     if (it == the_map.end()){
        throw(std::runtime_error(std::string("NamedObjMap::get : unknown id = ")+id));}
     return *(it->second);
    }

...
// invParam.tune = QUDA_TUNE_YES;
 ...
 invParam.cuda_prec_refinement_sloppy = QudaInfo::get_cuda_prec_sloppy();
 invParam.cuda_prec_precondition = QudaInfo::get_cuda_prec_sloppy();
// invParam.cuda_prec_precondition = QUDA_HALF_PRECISION;QudaInfo::get_cuda_prec_sloppy();
 invParam.cuda_prec_eigensolver = QudaInfo::get_cuda_prec();
 invParam.struct_size = sizeof(invParam);
}
...
void InverterInfo::set_info_gcr_multigrid(XMLHandler& xmlr)
{
 ...
 for (int level=0;level<(nlevels-1);++level){
    ...
    xmlsetQLIntVector(xmllevel,"XYZTBlockExtents",ivalues,ivalindex,LayoutInfo::Ndim,true,block_extents);
    ...
    if (!check_blocking(block_extents,localextents,errmsg)){
       string errmessage=string("Invalid level ")+make_string(level)+" block extents";
       for (int d=0;d<LayoutInfo::Ndim;++d){
          errmessage+=" "+make_string(block_extents[d]);}
       errmessage+=" in MultiGrid inverter: "+errmsg;
       throw(std::invalid_argument(errmessage));}
       // read remaining parameters
    xmlsetQLEnum(xmllevel,"NullSpaceDim",{"S","L"},ivalues,ivalindex,true,0);
    xmlsetQLInt(xmllevel,"NullSpaceSolveSteps",ivalues,ivalindex,true,500);
    xmlsetQLReal(xmllevel,"CoarseSolverTolerance",rvalues,rvalindex,true,0.25);
    xmlsetQLInt(xmllevel,"CoarseSolverMaxIterations",ivalues,ivalindex,true,50);
...
void InverterInfo::output_gcr_multigrid(XMLHandler& xmlout) const
{
 ...
 for (int level=0;level<(nlevels-1);++level){
    string tag="Level"; tag+=make_string(level);
    XMLHandler xmllevel(tag);
    xmllevel.put_child(xmloutputQLIntVector("XYZTBlockExtents",ivalues,ivalindex,LayoutInfo::Ndim));
    xmllevel.put_child(xmloutputQLEnum("NullSpaceDim",{"S","L"},ivalues,ivalindex));
    xmllevel.put_child(xmloutputQLInt("NullSpaceSolveSteps",ivalues,ivalindex));
 ...
 xmllevel.put_child(xmloutputQLReal("CoarseSolverTolerance",rvalues,rvalindex));
 xmllevel.put_child(xmloutputQLInt("CoarseSolverMaxIterations",ivalues,ivalindex));
 int deflate=ivalues[ivalindex]; ivalindex++;
 if (deflate){
    ...
    xmldeflate.put_child(xmloutputQLReal("CutoffEigenvalue",rvalues,rvalindex));
                                         ...
                                         const QuarkActionInfo& qactioninfo) const
{
 ...
 for (int level=0;level<(nlevels-1);++level){
    for (int d=0;d<LayoutInfo::Ndim;++d){
       blockextents[level][d]=ivalues[ivalindex]; ++ivalindex;}
    nullspacedim[level]=(ivalues[ivalindex]==0)?24:32; ++ivalindex;
    setup_solver_numsteps[level]=ivalues[ivalindex]; ++ivalindex;
    coarse_solver_tol[level]=rvalues[rvalindex]; ++rvalindex;
    coarse_solver_maxiter[level]=ivalues[ivalindex]; ++ivalindex;
    presmooth_num[level]=ivalues[ivalindex]; ++ivalindex;
    postsmooth_num[level]=ivalues[ivalindex]; ++ivalindex;}
                                                                    
 int level=nlevels-1;
 coarse_solver_tol[level]=rvalues[rvalindex]; ++rvalindex;
 coarse_solver_maxiter[level]=ivalues[ivalindex]; ++ivalindex;
 int coarsest_deflate=ivalues[ivalindex]; ivalindex++;
 ...
 if (coarsest_deflate){
    deflate_tol=rvalues[rvalindex]; ++rvalindex;
    deflate_maxits=ivalues[ivalindex]; ++ivalindex;
    deflate_nvectors=ivalues[ivalindex]; ++ivalindex;
    deflate_chebyshev=ivalues[ivalindex]; ++ivalindex;
    deflate_cutoff=rvalues[rvalindex]; ++rvalindex;}

 bool load_null_vectors=(ivalues[ivalindex])?true:false; ++ivalindex;
 ...
 mg_param.compute_null_vector=(load_null_vectors)?QUDA_COMPUTE_NULL_VECTOR_NO:QUDA_COMPUTE_NULL_VECTOR_YES;
 ...
 for (int level=0;level<nlevels;++level){
    ...
    mg_param.coarse_solver_ca_lambda_max[level] = -1.0;  // determine max by power method
    ...
    mg_param.smoother_solver_ca_lambda_max[level] = -1.0; // use power iterations
    ...
    if ((level<(nlevels-1))||(!coarsest_deflate)){
       ...
       QudaEigParam& coarse_deflate_param(QudaMGInfoPtr->coarse_deflate_param);  // to shorten notation
       coarse_deflate_param.use_poly_acc=(deflate_chebyshev>1)?QUDA_BOOLEAN_TRUE:QUDA_BOOLEAN_FALSE;
...
void InverterInfo::do_blocking(const vector<int>& extents, vector<int>& block_extents) const
{
 uint Nd=LayoutInfo::Ndim;
 const uint max_block_val=864;
 block_extents.resize(Nd);
 for (uint d=0;d<Nd;++d) block_extents[d]=1;
...
bool InverterInfo::check_blocking(std::vector<int>& block_extents, std::vector<int>& extents,
                                  std::string& error_message) const
{
 ...
 for (int d=0;d<LayoutInfo::Ndim;++d){
    ...
    if ((nextvalue*block_extents[d])!=(extents[d])){
       error_message=make_string(extents[d])+" not divisible by "+make_string(block_extents[d]);
       return false;}
 ...
 if ((prod<mg_blockprodmin)||(prod>mg_blockprodmax)){
    error_message="Block volume "+make_string(prod)+" outside range "
                  +make_string(mg_blockprodmin)+"-"+make_string(mg_blockprodmax);
    return false;}
 return true;
}

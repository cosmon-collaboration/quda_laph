...
class InverterInfo
{
  ...
  private:
    ...
    void set_info_cgne(XMLHandler& xmlr);

    void output_cgne(XMLHandler& xmlout) const;
 
    void setQudaInvertParam_cgne(QudaInvertParam& invParam) const;
    ...
    void do_blocking(const std::vector<int>& extents, std::vector<int>& block_extents) const;

    bool check_blocking(std::vector<int>& block_extents, std::vector<int>& extents,

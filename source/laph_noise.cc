...
LaphZnNoise::LaphZnNoise(int zn, const uint32_t& seed) : rng(seed), values(zn)
{
 ...
 for (int i=0;i<11;i++) rng.generate();
...
Array<cmplx> LaphZnNoise::generateLapHQuarkSourceForSink(
                      int Textent, int Nspin, int nEigs)
{
...
Array<cmplx> LaphZnNoise::generateLapHQuarkSource(
                     int timeValue, int Textent, int Nspin, int nEigs)
{
...
/* if (tbc==QuarkActionInfo::ZeroDirichlet){
    if ((timeValue==0)||(timeValue==(Textent-1))){
       QDPIO::cerr << "Cannot request source for time 0 or Nt-1 "
                   << "with zero Dirichlet quark time boundary conditions"<<std::endl;}
       QDP_abort(1);}*/
 return laph_noise;
}

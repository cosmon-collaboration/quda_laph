...
const IOFMHandler::pos_type IOFMHandler::data_start_pos =
    1 + IOFMHandler::ID_string_length;
...
const IOFMHandler::openmode_type IOFMHandler::IO_MODE_RDWR =
...
void IOFMHandler::delete_file() {
  ...
  if (global_mode)
    comm_broadcast(&errcode, sizeof(int), 0);
...
void IOFMHandler::file_seek(const off_type offset, const whence_type whence) {
  if (whence ==
    ...
    check_for_failure(
        MPI_File_seek(fh, fsize + offset, IOFMHandler::IO_SEEK_BEG),
        "Failure during seek");
  } else {
    check_for_failure(MPI_File_seek(fh, offset, whence), "Failure during seek");
  }
}
...
void IOFMHandler::writeCommon(const char *data, const size_t nbytes) {
  ...
  int errcode, count;
  if ((isPrimaryRank()) || (!global_mode)) {
    errcode = MPI_File_write_at(fh, currdisp, (char *)data, nbytes, MPI_BYTE,
                                &status);
    if (errcode == MPI_SUCCESS) {
      errcode = MPI_Get_count(&status, MPI_BYTE, &count);
      if (uint(count) != nbytes)
        errcode = MPI_ERR_COUNT;
    }
  }
  if (global_mode)
    comm_broadcast(&errcode, sizeof(int), 0);
...
void IOFMHandler::writeDistributed(const char *data,
  ...
  if (!global_mode)
    check_for_failure(IO_ERR_ACCESS,
                      "Distributed write failure: not global mode");
  if (read_mode) {
    read_mode = false;
    checksum = 0;
  }
  pos_type currdisp = file_tell();
  // set the file view
  check_for_failure(MPI_File_set_view(fh, currdisp, dinfo.getFileElemType(),
                                      dinfo.getFileViewType(), (char *)"native",
                                      finfo),
                    "Failure during MPI_File_set_view");
  MPI_Status status;
  int errcode, count;
  ...
  if (errcode == MPI_SUCCESS) {
    ...
    if (count != dinfo.getViewElementsThisRank())
      errcode = MPI_ERR_COUNT;
  }
  ...
  check_for_failure(
      MPI_File_set_view(fh, 0, MPI_BYTE, MPI_BYTE, (char *)"native", finfo),
      "Failure during MPI_File_set_view");
  check_for_failure(
      MPI_File_seek(fh, currdisp + dinfo.getViewTotalBytes(), MPI_SEEK_SET),
      "Failure during distributed write");
}
...
void IOFMHandler::readCommon(char *data, const size_t nbytes, const bool broadcast) {
  ...
  if ((global_mode) && (broadcast))
    comm_broadcast(data, nbytes, 0);
  check_for_failure(MPI_File_seek(fh, currdisp + nbytes, MPI_SEEK_SET),
...
void IOFMHandler::readDistributed(char *data,
  ...
  if (!global_mode)
    check_for_failure(IO_ERR_ACCESS,
                      "Distributed read failure: not global mode");
  if (!read_mode) {
    read_mode = true;
    checksum = 0;
  }
  ...
  check_for_failure(
      MPI_File_seek(fh, currdisp + dinfo.getViewTotalBytes(), MPI_SEEK_SET),
      "Failure during distributed read");
}
  ...
  for (size_t site = lexicostart; site < lexicostop; site += xinc) {
    ...
    if (thisrank == rank) {
      checksum = m_bytehandler.get_checksum(
          checksum, data + count * bytes_per_site, chunksize);
      count += xinc;
    }
    comm_barrier();
  }
...
#ifdef ARCH_PARALLEL
  if (global_mode)
    comm_broadcast(&endian_format, sizeof(char), 0);
#endif
...
void IOFMHandler::turnOffChecksum() { checksum_on = false; }

void IOFMHandler::resetChecksum() { checksum = 0; }

ByteHandler::n_uint32_t IOFMHandler::getChecksum() {
  if (checksum_on)
    return checksum;
  check_for_failure(
      true, "Invalid call to getChecksum since checksums not turned on");
...
void IOFMHandler::write_common(const char *output,
  ...
  if (endian_convert) {
    m_bytehandler.byte_swap(const_cast<char *>(output), elementbytes,
                            nelements);
    if (checksum_on)
...
//   Main input/output routines that do the byte-swapping (if needed),
//   update the check sum, and do the read/write.  "bytes_per_site" is the total
//   number of bytes needed to define the entire quantity on one lattice site
//   (includes color matrix, spin vectors, etc.).  "bytes_per_word" is the
//   number of bytes in the underlying numbers or words (int, double, complex,
...
void IOFMHandler::write_lattice(const char *output,
  ...
  if (endian_convert) {
    ...
    if (checksum_on)
      compute_lattice_checksum(b, dinfo.getBytesPerSite(),
...
void IOFMHandler::write(const LattField &output) {
  ...
  std::vector<unsigned int> sz(LayoutInfo::Ndim+2) ;
  ...
  write_lattice((const char *)(output.getDataConstPtr()), output.bytesPerWord(),
                lattinfo);
}
...
void IOFMHandler::read(LattField &input) {
  std::vector<unsigned int> sz(LayoutInfo::Ndim + 2);
  multi_read(sz.data(), LayoutInfo::Ndim + 2);
  ...
  }                          // done for IOMap to help with numbytes
...
size_t IOFMHandler::numbytes(const LattField &data) {
  return (LayoutInfo::Ndim + 2) * sizeof(int) +
         lattcmplxbytes * data.elemsPerSite() *
...
#ifdef ARCH_PARALLEL
  if (global_mode && (!broadcast) && checksum_on)
    comm_broadcast(&checksum, sizeof(checksum), 0);
#endif
  delete[] str;
}
...
//  ordering. Assumed distributed in a regular way...same size on each rank. No
//  distribution of the MPI ranks is assumed.  This class is mainly used by
//  BinaryFileHandler for creating file views for MPI-IO.
...
DistArrayViewInfo::DistArrayViewInfo(const DistArrayViewInfo &in)
    : ndim(in.ndim), global_sizes(in.global_sizes), local_sizes(in.local_sizes),
      start_indices(in.start_indices), nelem_this_rank(in.nelem_this_rank),
...
bool DistArrayViewInfo::checker(const std::vector<int> &gsizes,
  ...
  for (int k = 0; k < int(gsizes.size()); k++) {
    ...
    if (numranks[k] * ll != gsizes[k])
      return false;
  }
  int check = 1;
  for (int k = 0; k < int(gsizes.size()); k++)
    check *= numranks[k];
  if (check != int(comm_size()))
    return false;
  return true;
}
...
void DistArrayViewInfo::create_filetype() {
  ...
  error_return(MPI_Type_create_subarray(ndim, &global_sizes[0], &local_sizes[0],
                                        &start_indices[0], MPI_ORDER_FORTRAN,
                                        etype, &ftype),
...
DistArrayViewInfo::DistArrayViewInfo(const DistArrayViewInfo &in)
    : ndim(in.ndim), local_sizes(in.local_sizes), gvvol(in.gvvol),
      lexico_start(in.lexico_start), nbytes_per_element(in.nbytes_per_element) {
}

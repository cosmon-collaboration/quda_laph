...
#pragma omp parallel for
 for (int t=0;t<NT;++t){
  ...
  bool GaugeCERNConfigReader::read(std::vector<LattField>& u, const std::string& in_cfg_file)
  {
    const std::string cfg_file=tidyString(in_cfg_file);
    if (cfg_file.empty()){
    ...
    StopWatch rtimer, iotimer ; rtimer.start(); 
    ...
    double avgplaq=0.0;   
    const int dbsize=sizeof(double);
    iotimer.start();
...
#pragma omp parallel for
 for (int t=0;t<locNT;++t){
  ...
  void GaugeCERNConfigReader::local_cern_to_qdp_lexico_edge( const double* xedge, const double* yedge, const double* zedge,
...
bool GaugeCERNConfigReader::read(std::vector<LattField>& u, const std::string& in_cfg_file)
{
 ...
 const int nbytes_per_site=2*ndblesite*dbsize;  // only odd sites, but 8 directions
 ...
 int start_parity;
 std::vector<int> viewblockdisps, viewblocklengths;
 ...
 if (ts!=0){
    trecv.resize(tsdim);
    tr=trecv.data();
    std::vector<int> rank_coord(LayoutInfo::getMyCommCoords());  // rank coords of this node
...
bool GaugeSCIDACConfigReader::read(std::vector<LattField>& u, const std::string& in_cfg_file)
{
  const std::string cfg_file=tidyString(in_cfg_file);
  if (cfg_file.empty()){
 ...
 std::vector<char> lime_record_header(144);
 ...
 for (int irec=0;irec<5;++irec){
    ...
    std::vector<char> record_info(padded_length);
    iotimer.start();
 ...
 const size_t linkreals=2*FieldNcolor*FieldNcolor;
 const size_t ndir=LayoutInfo::Ndim;
 const size_t nelemsite=ndir*linkreals;
 const size_t nsites=NX*NY*NZ*NT;
 const size_t nelem=nsites*nelemsite;
 char prec='U';
 size_t rsize=0;
 if (precision==32){
 ...
 padded_length=lime_record_data_length+(7-((lime_record_data_length-1)%8));
 total_bytes+=144+padded_length;
 std::vector<char> record_data(padded_length);
 iotimer.start();
 ...
 // now rearrange links
 u.resize(ndir);
 const size_t sitesize=rsize*linkreals;
...
bool GaugeSCIDACConfigReader::read(std::vector<LattField>& u, const std::string& in_cfg_file)
{
 ...
 const size_t ndir=LayoutInfo::Ndim;
 const size_t nelemsite=ndir*linkreals;
 const size_t nsites=LayoutInfo::getLatticeNumSites();
 const size_t ranknsites=LayoutInfo::getRankLatticeNumSites();
 const size_t nelem=nsites*nelemsite;
 char prec='U';
 size_t rsize=0;
 if (precision==32){
 ...
 padded_length=lime_record_data_length+(7-((lime_record_data_length-1)%8));
 total_bytes+=144+padded_length;
 const int nbytes_per_site=nelemsite*rsize;  // links in the 4 directions are contiguous
 std::vector<int> starts(ndir);
 ...
 std::vector<char> buffer(ranknsites*nbytes_per_site);
 iotimer.start();
                                                     ...
                                                     std::string& lime_type)
{
 ...
 std::vector<char> lime_type_buffer(128);
                                                  ...
                                                  XMLHandler& record_xml)
{
  ...
  const size_t nchar=lime_record_info.size();
  size_t nullpos=0;
  bool intag=false;
  bool erase=false;
 for (int k=0;k<int(nchar);k++){
    ...
    if (lime_record_info[k]=='<'){
       ...
       if (erase){
          ...
          erase=true;}}}
 record_xml.set_from_string(std::string(lime_record_info.data(),nullpos));
}  
} // namespace LaphEnv

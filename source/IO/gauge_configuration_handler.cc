...
void GaugeConfigurationHandler::setInfo(
    const GaugeConfigurationInfo &gaugeinfo) {
  clear();
  set_info(gaugeinfo);
}

void GaugeConfigurationHandler::set_info(
    const GaugeConfigurationInfo &gaugeinfo) {
  try {
...
GaugeConfigurationHandler::~GaugeConfigurationHandler() { clear(); }
...
GaugeConfigurationHandler::getGaugeConfigurationInfo() const {
...
void GaugeConfigurationHandler::initialize_config() {
  try {
    ...
    errorLaph(
        make_strf("Gauge configuration initialization failed: %s", xp.what()));
  }
}
...
void GaugeConfigurationHandler::applyFermionTemporalAntiPeriodic() {
  ...
  check_info_set("getData");
  if (!isDataSet()) {
    errorLaph("Gauge configuration must already be in the NamedObjMap to apply "
  ...
  try {
    ...
    errorLaph(
        make_strf("applyFermionTemporalAntiPeriodic failed: %s", xp.what()));
  }
}
...
void GaugeConfigurationHandler::copyDataToDevice(bool removeFromHost) {
  ...
  printLaph(
      make_strf(" Computed mean plaquette = %15.12f", param.plaquette[0]));
  printLaph(
      make_strf("  spatial mean plaquette = %15.12f", param.plaquette[1]));
  printLaph(
      make_strf(" temporal mean plaquette = %15.12f", param.plaquette[2]));

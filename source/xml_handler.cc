                        ...
                        const string& rootcontent)
{
 try{
    ...
    clear();
    errorLaph(make_strf("Serious XML document error: fatal  %s\n",msg.what()));}
}
...
void XMLHandler::seek_next(const std::string& tagname)
{
 ...
 while ((current!=0)&&((current->text)||(current->name!=tagname)))
    seek_next_node();
 if ((current==0)&&(exceptions))
    throw(std::invalid_argument(string("Seek next node by tag name ")+tagname
             +string(" failed")));
}
...
{
 if (exceptions) throw(std::invalid_argument("err"));
 return string();
}
...
void XMLHandler::put_sibling_text_node(const string& text)
{
 try{
    ...
    if (branch==0) throw(std::invalid_argument("err"));
    content->connect_as_nextsibling(current,branch);
    seek_next_sibling();}
 catch(const std::exception& msg){
...
{
 try{
    if ((fail())||(current->text)) throw(std::invalid_argument("err"));
...
void xmlreadfail(XMLHandler& xmlh, const std::string& infoname, const string& message)
{

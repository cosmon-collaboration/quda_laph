...
void QuarkSmearingHandler::applyLaphPhaseConvention(
  ...
  comm_coords[LayoutInfo::Ndim - 1] =
      LayoutInfo::getMyCommCoords()[LayoutInfo::Ndim - 1];
  comm_coords[0] = 0;
  comm_coords[1] = 0;
  comm_coords[2] = 0;
  ...
  int count = 0;
  for (comm_coords[0] = 0;
       comm_coords[0] < LayoutInfo::getCommNumPartitions()[0]; ++comm_coords[0])
    for (comm_coords[1] = 0;
  ...
  if (broadcast_ranks.size() > 1) {
    if (int(broadcast_ranks.size()) == LayoutInfo::getNumRanks()) {
      ...
      if (time_comm != MPI_COMM_NULL) {
        int status = MPI_Bcast(rephase.data(), nev * nloctime * cbytes,

...
#ifdef ARCH_PARALLEL
 if (global_mode) comm_broadcast(&result, sizeof(bool), 0);
#endif
 return result;
}
...
bool globalAnd(const bool& flag)
{
 ...
 bool result=flag;
 int status=MPI_Allreduce(&flag,&result,1,MPI_C_BOOL,MPI_LAND,MPI_COMM_WORLD);
...
#elif defined(ARCH_PARALLEL)
 uint result=uival;
 int status=MPI_Allreduce(&uival,&result,1,MPI_UNSIGNED,MPI_MAX,MPI_COMM_WORLD);
 if (status!=MPI_SUCCESS){
...
#elif defined(ARCH_PARALLEL)
 float result=rval;
 int status=MPI_Allreduce(&rval,&result,1,MPI_FLOAT,MPI_MAX,MPI_COMM_WORLD);
 if (status!=MPI_SUCCESS){
    errorLaph("Problem occurred in globalMax");}
 return result;
#endif
...
#elif defined(ARCH_PARALLEL)
 double result=rval;
 int status=MPI_Allreduce(&rval,&result,1,MPI_DOUBLE,MPI_MAX,MPI_COMM_WORLD);
 if (status!=MPI_SUCCESS){
    errorLaph("Problem occurred in globalMax");}
 return result;
#endif
...
#elif defined(ARCH_PARALLEL)
 uint result=uival;
 int status=MPI_Allreduce(&uival,&result,1,MPI_UNSIGNED,MPI_MIN,MPI_COMM_WORLD);
 if (status!=MPI_SUCCESS){
    errorLaph("Problem occurred in globalMax");}
 return result;
#endif
...
// *************************************************************************
     ...
     // Returns true if integer "ival" has the same value on all ranks; false otherwise.
...
bool isIntegerSameAllRanks(const int& ival) 
{
 ...
 p[0] = -ival;
 p[1] = ival;
 int status=MPI_Allreduce(MPI_IN_PLACE, p, 2, MPI_INT, MPI_MIN, MPI_COMM_WORLD);
 if (status!=MPI_SUCCESS){
...
bool isBooleanSameAllRanks(bool bval) 
{
 ...
 bool check=bval;
 bool result;
 int status=MPI_Allreduce(&check,&result,1,MPI_C_BOOL,MPI_LAND,MPI_COMM_WORLD);
 if (status!=MPI_SUCCESS){
    errorLaph("Problem occurred in globalAnd");}
 if (result) return true;
 check=!bval;
 status=MPI_Allreduce(&check,&result,1,MPI_C_BOOL,MPI_LAND,MPI_COMM_WORLD);
 if (status!=MPI_SUCCESS){
    errorLaph("Problem occurred in globalAnd");}
 return result;
#endif
...
void commSendRecv(void *buffer, size_t nbytes, int send_rank, int recv_rank)
{
 ...
 if (myrank==send_rank){
    ...
    int status=MPI_Recv(buffer,nbytes,MPI_CHAR,send_rank,0,MPI_COMM_WORLD,&statusinfo);
    if (status!=MPI_SUCCESS){
...
int comm_rank()
{
 return quda::comm_rank();
}


int commCoords(int rank)
{
 return quda::commCoords(rank);
}
...
void xmlsetQLInt(XMLHandler& xmlin, const std::string& tagname, 
                 vector<int>& ivalues, int& index,
                 bool optional, int default_value)
{
...
void xmlsetQLBool(XMLHandler& xmlin, const std::string& tagname, 
                  vector<int>& ivalues, int& index,
                  bool optional, bool default_value)
{
 if (index>=int(ivalues.size())){
    throw(std::runtime_error("ivalues vector not large enough for QL bool set"));}
 XMLHandler xmlh(xmlin);
 try{
    bool state=xmlh.get_exceptions();
    xmlh.set_exceptions_on();
...
void xmlsetQLReal(XMLHandler& xmlin, const std::string& tagname, 
                  vector<double>& rvalues, int& index,
                  bool optional, double default_value)
{
 if (index>=int(rvalues.size())){
    throw(std::runtime_error("rvalues vector not large enough for QL real set"));}
 XMLHandler xmlh(xmlin);
 try{
    bool state=xmlh.get_exceptions();
    xmlh.set_exceptions_on();
                       ...
                       bool optional, const vector<int>& default_value)
{
 if (index>=(int(ivalues.size())-nvalues+1)){
    throw(std::runtime_error("ivalues vector not large enough for QL int vector set"));}
 ...
 try{
    ...
    if (nvalues!=int(values_read.size())){
       throw(std::invalid_argument("Number of values is incorrect for int vector read"));}
    for (int j=0;j<nvalues;++j){
       ivalues[index]=values_read[j]; ++index;}
    xmlh.set_exceptions(state);}
 catch(const std::exception& err_msg){
    if (optional){
       for (int j=0;j<nvalues;++j){
          ivalues[index]=default_value[j]; ++index;}}
    else{
...
void xmlsetQLEnum(XMLHandler& xmlin, const std::string& tagname,
                  ...
                  vector<int>& ivalues, int& index,
                  bool optional, int default_index)
{
 if (index>=int(ivalues.size())){
    throw(std::runtime_error("ivalues vector not large enough for QL enum set"));}
 XMLHandler xmlh(xmlin);
 bool invalid=false;
 try{
    bool state=xmlh.get_exceptions();
    ...
    for (uint j=0;j<xmlchoices.size();++j){
       if (content==xmlchoices[j]){
          ivalues[index]=j; invalid=false; break;}}
    if (invalid){
...
XMLHandler xmloutputQLInt(const std::string& tagname, const vector<int>& ivalues, int& index)
{
 if (index>=int(ivalues.size())){
    throw(std::runtime_error("ivalues vector not large enough for QL output"));}
 string val(make_string(ivalues[index])); ++index;
 return XMLHandler(tagname,val);
}

   //  Same as the above routine, except the value is of type bool.

XMLHandler xmloutputQLBool(const std::string& tagname, const vector<int>& ivalues, int& index)
{
 if (index>=int(ivalues.size())){
    throw(std::runtime_error("ivalues vector not large enough for QL output"));}
...
XMLHandler xmloutputQLReal(const std::string& tagname, const vector<double>& rvalues, int& index)
{
 if (index>=int(rvalues.size())){
    throw(std::runtime_error("rvalues vector not large enough for QL output"));}
 string val(make_string(rvalues[index])); ++index;
 return XMLHandler(tagname,val);
}

XMLHandler xmloutputQLIntVector(const std::string& tagname, const vector<int>& ivalues, 
...
XMLHandler xmloutputQLEnum(const std::string& tagname, const vector<string>& xmlchoices,
                           const vector<int>& ivalues, int& index)
{
 if (index>=int(ivalues.size())){
    throw(std::runtime_error("ivalues vector not large enough for QL output"));}
 string val(xmlchoices[ivalues[index]]); ++index;
 return XMLHandler(tagname,val);
}
...
int xmlputQLInt(const std::string& tagname, const vector<int>& ivalues, int& index)
{
 if (index>=int(ivalues.size())){
...
bool xmlputQLBool(const std::string& tagname, const vector<int>& ivalues, int& index)
{
 if (index>=int(ivalues.size())){
    throw(std::runtime_error("ivalues vector not large enough for QL put"));}
...
double xmlputQLReal(const std::string& tagname, const vector<double>& rvalues, int& index)
{
 if (index>=int(rvalues.size())){
...
vector<int> xmlputQLIntVector(const std::string& tagname, const vector<int>& ivalues, int& index,
                              int nvalues)
{
 if (index>=(int(ivalues.size())-nvalues+1)){
    throw(std::runtime_error("ivalues vector not large enough for QL put"));}

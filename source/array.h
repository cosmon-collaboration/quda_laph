...
class Array {
 ...
 private:
   ...
   unsigned int serialindex(const std::vector<unsigned int>& ind, unsigned int istart) const;
...
// if (m_sizes.size()!=1) throw(std::invalid_argument("Array: improper rank of array indices"));
 if ((i<0)||(i>=int(m_store.size()))) throw(std::invalid_argument("Array: index out of range"));
#endif
 return m_store[i];
}
...
template <typename T>
inline const T& Array<T>::get(unsigned int off) const
{
 return m_store[off];
}

...
{  
  ...
  size_t idx = 0 ;
  std::vector<std::vector<std::vector<std::vector< size_t >>>> mapEv(nEv) ;
  for( int aEv = 0 ; aEv < nEv ; aEv++ ) {
    mapEv[aEv].resize( nEv ) ;
    for( int bEv = 0 ; bEv < nEv ; bEv++ ) {
      mapEv[aEv][bEv].resize( nEv ) ;
      for( int cEv = 0 ; cEv < nEv ; cEv++ ) {
	mapEv[aEv][bEv][cEv].resize( nEv ) ;
	for( int dEv = 0 ; dEv < nEv ; dEv++ ) {
	  if( dEv > cEv && cEv > bEv && bEv > aEv ) {
	    mapEv[aEv][bEv][cEv][dEv] = idx ; idx++ ;
	  } else {
	    mapEv[aEv][bEv][cEv][dEv] = -1 ;
	  }
	}
      }
    }
...
#pragma omp parallel for collapse(2)
  for( int aEv = 0 ; aEv < nEv ; aEv++ ) {
    for( int bEv = aEv+1 ; bEv < nEv ; bEv++ ) {
      ...
      cpuColorCross( host_evec[aEv] , host_evec[bEv] , (void*)Diq1.getDataPtr() , X ) ;
      for( int cEv = bEv+1 ; cEv < nEv ; cEv++ ) {
	for( int dEv = cEv+1 ; dEv < nEv ; dEv++ ) {

	  cpuColorCross( host_evec[cEv] , host_evec[dEv] , (void*)Diq2.getDataPtr() , X ) ;
	      ...
	      const int X[4])
{
  ...
  //}
  if( (size_t)blockSizeMomProj > nEvChoose4 ) { errorQuda("blockSizeMomProj too big %d > %zu" , blockSizeMomProj , nEvChoose4 ) ; }
  ...
  const size_t data_tmp_bytes = std::max( nDiq , blockSizeMomProj )*nSites*2*precision;
  ...
  cublas_param_mom_sum.trans_b = QUDA_BLAS_OP_C;
  ...
  for (int aEv=0; aEv<nEv; aEv++) {
	...
	while( dEv < nEv ) {
	  ...
	  const int diqBlk2 = std::min( std::min( nEv - dEv , nDiq ) , (blockSizeMomProj-nInBlock ) ) ; ;
...
int main(int argc, char *argv[]) {
  ...
  const size_t nEvChoose4 = Nev*(size_t)(Nev-1)/2*(Nev-2)/3*(Nev-3)/4;
  double _Complex *retGPU = (double _Complex*)calloc( X[3]*nmom*nEvChoose4 , sizeof( double _Complex  ) );
  ...
  double _Complex *retCPU = (double _Complex*)calloc( X[3]*nmom*(size_t)nEvChoose4 , sizeof( double _Complex  ) );
  StopWatch cpu ;

  printf( "Alloc retCPU %e GB\n" , X[3]*nmom*(size_t)nEvChoose4/(1024*1024*1024.) ) ;
  cpu.start() ;
  ...
  for( size_t p = 0 ; p < nmom ; p++ ) {
    double sum = 0 ;
    for( int T = 0 ; T < X[3] ; T++ ) {
      for( size_t i = 0 ; i < nEvChoose4 ; i++ ) {
	const size_t idx = T + X[3]*(i + nEvChoose4*p) ;
    ...
    std::cout<<"Summed diff p="<<p<<" "<<sum/(nEvChoose4*X[3])<<std::endl ;
  }
  free( retCPU ) ;
#endif
  free( retGPU ) ;
